			...
			<div class="panel">
				...
				<div class="search-panel">
					<div class="row" >
						...
						<div class="col-sm-3"><strong>Discount Type:</strong>&nbsp;<br>
							<select name="dc_type" id="dc_type" onchange="getShippingAppliedOnOption('2');" class="form-control">
								<?php echo $obj->getDiscountCouponTypeOption($dc_type,2); ?>
							</select>
						</div>
						<div class="col-sm-3"><strong>Discount Applied On:</strong>&nbsp;<br>
							<select name="dc_applied_on" id="dc_applied_on" class="form-control">
								<?php echo $obj->getDiscountCouponAppliedOnOption($dc_type,$dc_applied_on,2); ?>
							</select>
						</div>
					</div>
					<div class="row" >
					...
					<div class="row" >	
						<div class="col-sm-3">
							...
							<input type="text" name="dc_effective_date" id="dc_effective_date" placeholder="Select Effective Date" class="form-control">
						</div>
						<div class="col-sm-3">	
				...
				<div class="panel-body">
					...
					if($obj->chkIfAccessOfMenuAction($admin_id,$view_action_id))
					{ ?>
					<div id="discountcouponslist"></div>
					<?php
					} ?>
				</div>
			</div>
...
{ 
	...
	$('#dc_effective_date').datepicker();
	<?php
	if($obj->chkIfAccessOfMenuAction($admin_id,$view_action_id))
	{ ?>
    var dataString ='action=discountcouponslist';
	$.ajax({
		type: "POST",
		url: "ajax/remote.php",
		data: dataString,
		...
		{
			$("#discountcouponslist").html(result);
		}
	});
	<?php
	} ?>
...
function change_page(page_id)  
{
	...
	var dc_effective_date = $("#dc_effective_date").val();
	
	var dataString ='page_id='+page_id +'&txtsearch='+escape(txtsearch)+'&status='+status+'&dc_type='+dc_type+'&dc_applied_on='+dc_applied_on+'&country_id='+country_id+'&state_id='+state_id+'&city_id='+city_id+'&area_id='+area_id+'&added_by_admin='+escape(added_by_admin)+'&added_date_type='+escape(added_date_type)+'&added_days_of_month='+escape(added_days_of_month)+'&added_days_of_week='+escape(added_days_of_week)+'&added_single_date='+escape(added_single_date)+'&added_start_date='+escape(added_start_date)+'&added_end_date='+escape(added_end_date)+'&dc_effective_date='+escape(dc_effective_date)+'&action=discountcouponslist';
	$.ajax({
		type: "POST",
		url: "ajax/remote.php",
		data: dataString,
...
function refineSearch()  
{
	...
	var dataString ='txtsearch='+escape(txtsearch)+'&status='+status+'&dc_type='+dc_type+'&dc_applied_on='+dc_applied_on+'&country_id='+country_id+'&state_id='+state_id+'&city_id='+city_id+'&area_id='+area_id+'&added_by_admin='+escape(added_by_admin)+'&added_date_type='+escape(added_date_type)+'&added_days_of_month='+escape(added_days_of_month)+'&added_days_of_week='+escape(added_days_of_week)+'&added_single_date='+escape(added_single_date)+'&added_start_date='+escape(added_start_date)+'&added_end_date='+escape(added_end_date)+'&dc_effective_date='+escape(dc_effective_date)+'&action=discountcouponslist';
	$.ajax({
		type: "POST",
		url: "ajax/remote.php",
		data: dataString,
...
{ ?>	
function deletemultiplediscountcoupons()
{
	var favorite = [];
	var chkbox_records = "";
	...
	else
	{
		...
		if (r == true) 
		{
			var dataString ='chkbox_records='+chkbox_records+'&action=deletemultiplediscountcoupons';
