			...
			<div class="panel">
				...
				<div class="search-panel">
					<div class="row" >
						...
						<div class="col-sm-3"><strong>Shipping Type:</strong>&nbsp;<br>
							<select name="sp_type" id="sp_type" onchange="getShippingAppliedOnOption('2');" class="form-control">
								<?php echo $obj->getShippingTypeOption($sp_type,2); ?>
							</select>
						</div>
						<div class="col-sm-3"><strong>Shipping Applied On:</strong>&nbsp;<br>
							<select name="sp_applied_on" id="sp_applied_on" class="form-control">
								<?php echo $obj->getShippingAppliedOnOption($sp_type,$sp_applied_on,2); ?>
							</select>
						</div>
					</div>
					<div class="row" >
					...
					<div class="row" >	
						<div class="col-sm-3">
							...
							<input type="text" name="sp_effective_date" id="sp_effective_date" placeholder="Select Effective Date" class="form-control">
						</div>
						<div class="col-sm-3">	
...
{ 
	...
	if($obj->chkIfAccessOfMenuAction($adm_vendor_id,$view_action_id))
	{ ?>
    var dataString ='action=shippingpriceslist';
	$.ajax({
		type: "POST",
		url: "ajax/remote.php",
		data: dataString,
		...
		{
			$("#shippingpriceslist").html(result);
		}
	});
	<?php
	} ?>
...
function change_page(page_id)  
{
	...
	var sp_effective_date = $("#sp_effective_date").val();
	
	var dataString ='page_id='+page_id +'&txtsearch='+escape(txtsearch)+'&status='+status+'&sp_type='+sp_type+'&sp_applied_on='+sp_applied_on+'&country_id='+country_id+'&state_id='+state_id+'&city_id='+city_id+'&area_id='+area_id+'&added_by_admin='+escape(added_by_admin)+'&added_date_type='+escape(added_date_type)+'&added_days_of_month='+escape(added_days_of_month)+'&added_days_of_week='+escape(added_days_of_week)+'&added_single_date='+escape(added_single_date)+'&added_start_date='+escape(added_start_date)+'&added_end_date='+escape(added_end_date)+'&sp_effective_date='+escape(sp_effective_date)+'&action=shippingpriceslist';
	$.ajax({
		type: "POST",
		url: "ajax/remote.php",
		data: dataString,
...
function refineSearch()  
{
	...
	var dataString ='txtsearch='+escape(txtsearch)+'&status='+status+'&sp_type='+sp_type+'&sp_applied_on='+sp_applied_on+'&country_id='+country_id+'&state_id='+state_id+'&city_id='+city_id+'&area_id='+area_id+'&added_by_admin='+escape(added_by_admin)+'&added_date_type='+escape(added_date_type)+'&added_days_of_month='+escape(added_days_of_month)+'&added_days_of_week='+escape(added_days_of_week)+'&added_single_date='+escape(added_single_date)+'&added_start_date='+escape(added_start_date)+'&added_end_date='+escape(added_end_date)+'&sp_effective_date='+escape(sp_effective_date)+'&action=shippingpriceslist';
	$.ajax({
		type: "POST",
		url: "ajax/remote.php",
		data: dataString,
...
{ ?>	
function deletemultipleshippingprices()
{
	var favorite = [];
	var chkbox_records = "";
	...
	else
	{
		...
		if (r == true) 
		{
			var dataString ='chkbox_records='+chkbox_records+'&action=deletemultipleshippingprices';
