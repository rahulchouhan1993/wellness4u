...
$obj->debuglog('[EDIT_CUISINE] REQUEST:<pre>'.print_r($_REQUEST,true).'</pre>, FILES:<pre>'.print_r($_FILES,true).'</pre>');
...
{
	$cusine_id = trim($_POST['hdncusine_id']);
	...
	//$cusine_status = strip_tags(trim($_POST['cusine_status']));
	$cusine_status = '';
	$hdncusine_image = strip_tags(trim($_POST['hdncusine_image']));
	...
	if(!$error)
	{
		...
		if (!empty($_FILES) && (isset($_FILES['cusine_image']['name'])) && ($_FILES['cusine_image']['name'] != '') )
		{
	...
	if(!$error)
	{
		...
		$tdata['cusine_status'] = $cusine_status;
		$tdata['modified_by_admin'] = 0;
		
		if($obj->updateCusine($tdata))
		{
