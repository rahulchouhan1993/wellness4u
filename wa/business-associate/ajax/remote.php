...
if ($action == 'getstate') {
    ...
    */
    ...
    $item = $obj->getAllEvents($txtsearch, $status, $cucat_parent_cat_id, $cucat_cat_id, $vendor_id, $added_start_date, $added_end_date);
    ...
    if (is_array($data) && count($data) > 0) {
        foreach ($data as $record) {
            ...
            $option.= '<td>' . $record['event_name'] . '</td>
                                ...
                                if($obj->chkIfShowVendorAccessFormFieldFromVAFID($vaf_id,'prof_cat_id_1'))
                                 {
                                   $option.= '<td>' . $obj->getProfileCategoryName($record['prof_cat_id_1']) . '</td>'; 
                                 }

                                 if($obj->chkIfShowVendorAccessFormFieldFromVAFID($vaf_id,'fav_cat_id_1'))
                                 {
                                   $option.= '<td>' . $obj->getIdByProfileFavCategoryName($record['fav_cat_id_1']) . '</td>'; 
                                 }

                                 if($obj->chkIfShowVendorAccessFormFieldFromVAFID($vaf_id,'prof_cat_id_2'))
                                 {
                                   $option.= '<td>' . $obj->getProfileCategoryName($record['prof_cat_id_2']) . '</td>'; 
                                 }

                                 if($obj->chkIfShowVendorAccessFormFieldFromVAFID($vaf_id,'fav_cat_id_2'))
                                 {
                                   $option.= '<td>' . $obj->getIdByProfileFavCategoryName($record['fav_cat_id_2']) . '</td>'; 
                                 }


                                 if($obj->chkIfShowVendorAccessFormFieldFromVAFID($vaf_id,'prof_cat_id_3'))
                                 {
                                   $option.= '<td>' . $obj->getProfileCategoryName($record['prof_cat_id_3']) . '</td>'; 
                                 }

                                 if($obj->chkIfShowVendorAccessFormFieldFromVAFID($vaf_id,'fav_cat_id_3'))
                                 {
                                   $option.= ' <td>' . $obj->getIdByProfileFavCategoryName($record['fav_cat_id_3']) . '</td>'; 
                                 }

                                 if($obj->chkIfShowVendorAccessFormFieldFromVAFID($vaf_id,'prof_cat_id_4'))
                                 {
                                   $option.= '<td>' . $obj->getProfileCategoryName($record['prof_cat_id_4']) . '</td>'; 
                                 }

                                 if($obj->chkIfShowVendorAccessFormFieldFromVAFID($vaf_id,'fav_cat_id_4'))
                                 {
                                   $option.= '<td>' . $obj->getIdByProfileFavCategoryName($record['fav_cat_id_4']) . '</td>'; 
                                 }
                                

                                 if($obj->chkIfShowVendorAccessFormFieldFromVAFID($vaf_id,'prof_cat_id_5'))
                                 {
                                   $option.= '<td>' . $obj->getProfileCategoryName($record['prof_cat_id_5']) . '</td>'; 
                                 }

                                 if($obj->chkIfShowVendorAccessFormFieldFromVAFID($vaf_id,'fav_cat_id_5'))
                                 {
                                   $option.= '<td>' . $obj->getIdByProfileFavCategoryName($record['fav_cat_id_5']) . '</td>'; 
                                 }

                               $option.= ' <td>' . $obj->getVendorName($record['organiser_id']) . '</td>
    ...
    $event = $obj->GetAllEventDetails($txtsearch, $status, $parent_cat_id, $sub_cat_id, $country_id, $state_id, $city_id, $area_id, $admin_id);
    $option = '';
    //start pagination for notification
    $adjacents = 1;
    $records_per_page = 20;
    ...
    $vendor_id = $_SESSION['adm_vendor_id'];
    //$item = $obj->getAllAdviserUserReferrals_OLD($vendor_id, $user_id, $status, $invite_start_date, $invite_end_date, $status_start_date, $status_end_date);
        ...
        foreach ($data as $record) {
            ...
            $option.= '<tr>
                                            ...
                                            <td height="30" align="center" valign="middle" bgcolor="#FFFFFF">' . $action_btn . '
                                                ...
                                                <a href="'.MAIN_URL.'my_life_pattrns.php?user_id=' . $uid . '" target="_blank"><input type="button" name="btnViewUserReport" id="btnViewUserReport" value="View User Report" > </a>
        ...
        if (count($data) > 0) {
            foreach ($data as $record) {
                ...
                if ($record['request_status'] == '1') {
                    ...
                    $action2 = '<input type="button" name="btnDecline" id="btnDecline" value="Deactivate" onclick="showDeactivateUserInvitationPopup(\'' . $record['ar_id'] . '\',\'' . $uid . '\',\'' . $pro_user_id . '\')">';
                    $time = strtotime($record['request_accept_date']);
                    $time = $time + 19800;
                    ...
                    $action2 = '<input type="button" name="btnDecline" id="btnDecline" value="Activate" onclick="showActivateUserInvitationPopup(\'' . $record['ar_id'] . '\',\'' . $uid . '\',\'' . $pro_user_id . '\')">';
                    $time = strtotime($record['request_accept_date']);
                    $time = $time + 19800;
                    $request_accept_date = date('d/m/Y h:i A', $time);
                ...
                $option.= '<table width="760" border="0" align="center" cellpadding="0" cellspacing="0">
                                        ...
                                        <table width="760" border="0" align="center" cellpadding="0" cellspacing="0">
                                            ...
                                            <tr>
                                                ...
                                                <td width="50%" align="left" valign="middle"><strong>User Name: ' . $record['name'] . '</strong>&nbsp;&nbsp;&nbsp;<input type="button" name="btnViewAdviserReportLog" id="btnViewAdviserReportLog" value="View Users Activity" onclick="javascript:window.open(\'my_user_activity.php?id=' . $uid . '\', \'_blank\');" ></td>
                                                 ...
                                                 <a href="#" onclick="var myArray = { user_id : '.$uid.'}; post2blank(\'my_users_queries.php\' , myArray);" target="_blank" style="color: #111;"><input class="btn-btn" type="button" name="btn" id="btn" value="Online Guidance" autocomplete="false"></a></td>
                ...
                $arr_report_permission = $obj->getAdviserReportsPermissions($uid, $pro_user_id, $record['ar_id']);
                if (count($arr_report_permission) > 0) {
                    ...
                    foreach ($arr_report_permission as $report_record) {
                        ...
                        $option.= '<tr>
                                                ...
                                                <td height="30" align="center" valign="middle" bgcolor="#FFFFFF"><a href="'.MAIN_URL.'my_life_pattrns.php?user_id=' . $uid . '" target="_blank">' . $obj->getReportTypeNameString($report_record['report_id'], $report_record['permission_type']) . '</a></td>
        ...
        if (count($data) > 0) {
            ...
            foreach ($data as $record) {
                ...
                if ($record['request_status'] == '1') {
                    ...
                    $action3='<input class="" type="button" name="btnReply" id="btnReply" style="width: 125px;" value="Add Inputs" onclick="showUserQueryPopup(\'' . "". '\',\'' . $record['user_id'] . '\')"/> ';
                ...
                <tr>
                    ...
                    <td width="50%" align="left" valign="middle"><strong>User Name:' . $obj->getUserFullNameById($record['user_id']) . '</strong>&nbsp;&nbsp;&nbsp;<input type="button" name="btnViewAdviserReportLog" id="btnViewAdviserReportLog" value="View Access Log" onclick="javascript:window.open(\'my_user_authorise_log.php?id=' . $record['user_id'] . '\', \'_blank\')" ></td>
                    ...
                    <a href="#" onclick="var myArray = { user_id : '.$record['user_id'].'}; post2blank(\'my_users_queries.php\' , myArray);" target="_blank" style="color: #111;"><input class="btn-btn" type="button" name="btn" id="btn" value="Online Guidance" autocomplete="false"></a>
                         </td>

                    <td width="25%" align="center" valign="middle"> ' . $action3 . '&nbsp; &nbsp; ' . $action2 . '</td>
    ...
    $user_id = stripslashes($_REQUEST['page_id']);
    $output = '';
    $vendor_id = $_SESSION['adm_vendor_id'];
    $output = $obj->showUserQueryPopup($vendor_id, $parent_aq_id,$user_id);
    echo $output;
} 
...
elseif ($action == 'showSelfQueryPopup') {
    ...
    $output = '';
        ...
        $output.= '<input type="hidden" name="parent_aq_id" id="parent_aq_id" value="' . $parent_aq_id . '" />'; 
        $output.= '<table cellpadding="0" cellspacing="0" width="75%" align="center" border="0">';

        $output.='<tr><td width="60%" height="40" align="left" valign="top">Reference:</td><td width="40%" height="40" align="left" valign="top">' . $obj->getReferenceName($temp_page_id,$temp_page_tbl) . '</td></tr>';
        $output.='<tr><td width="60%" height="40" align="left" valign="top">User:</td><td width="40%" height="40" align="left" valign="top">' . $name . '</td></tr>';
        $output.='<tr><td width="60%" height="40" align="left" valign="top">Last Query:</td><td width="40%" height="40" align="left" valign="top">' . $temp_query . '</td></tr>';
        $output.='<tr><td width="60%" height="110" align="left" valign="top">Update Query:</td><td width="40%" height="110" align="left" valign="top">
        <textarea  cols="30" rows="5" type="text" id="feedback_new" name="feedback_new"></textarea>
        <textarea  cols="30" rows="5" type="text" id="feedback_old" name="feedback_old" style="display:none;">' . $temp_query . '</textarea>
        </td></tr>';
        $output.='<tr><td width="60%" height="40" align="left" valign="middle">&nbsp;</td><td width="40%" height="40" align="left" valign="middle"><input name="submit" type="button" class="button" id="submit" value="Submit"  onclick="updateUserQuery()"/></td></tr>';
        $output.='</table>
                </form>';
...
elseif ($action == 'updateUserQuery') {
    ...
    echo $obj->updateUserQuery($aq_id, $query);
}
elseif ($action == 'showactivateuserinvitationpopup') {
    ...
    $user_id = stripslashes($_REQUEST['puid']);
    $status_reason = stripslashes($_REQUEST['status_reason']);
    $error = '0';
    $err_msg = '';
    $output = '';
    $vendor = stripslashes($_REQUEST['vendor']);
    $output = $obj->deactivateUserInvitation($ar_id, $user_id, $vendor, $status_reason);
    ...
    $output = '';
    $vendor = stripslashes($_REQUEST['vendor']);
    $output = $obj->activateUserInvitation($ar_id, $user_id, $vendor, $status_reason);
    echo $output;
...
elseif($action == 'togglereadunreadquery')
{
    ...
    if($tgaction == 'read')
    {
        $output=$obj->setUserQueryRead($aq_id,$vendor_id,'1');
    }
    elseif($tgaction == 'unread')
    {
        $output=$obj->setUserQueryRead($aq_id,$vendor_id,'0');
    }   
    echo $output;
}
//add by ample for user profile popup 24-03-20
...
elseif ($action == 'FormFeedbackSave') {

    $page_id = stripslashes($_REQUEST['page_id']);
    $feedback = stripslashes($_REQUEST['feedback']);
    $user_id = $_SESSION['adm_vendor_id'];
    $name = $_SESSION['adm_vendor_name'];
    $email = $_SESSION['adm_vendor_email'];
    $parent_id = 0;

    $output=$obj->InsertFeedback($parent_id, $page_id, $name, $email, $feedback, $user_id);
...
elseif ($action == 'feedback_reply_save') {
    ...
    $user_id=$_SESSION['adm_vendor_id'];
    $name = $obj->getVendorFullNameById($user_id);
    $email = $obj->getVendorEmailById($user_id);

    echo $obj->InsertFeedback($parent_id, $page_id, $name, $email, $feedback, $user_id);
...
elseif($action=="getVendorContacts")
{   
    $vendor_id=$_SESSION['adm_vendor_id'];
    $vloc_id = stripslashes($_REQUEST['vloc_id']);

    $data=$obj->get_vendor_contacts($vendor_id, $vloc_id);
    ...
    {   
        ...
        foreach ($data as $key => $value) {
            ...
            $html.='<tr>
                        ...
                        <td>'.$value["contact_person_title"].'</td>
                        <td>'.$obj->getSubCategoryName($value["contact_designation"]).'</td>
                        <td>
                            <a href="update_staff.php?SID='.$value["contact_id"].'" target="blank"><button class="btn btn-warning btn-sm">Update Profile</button></a>
                            <a href="session_time.php?SID='.$value["contact_id"].'" target="blank"><button class="btn btn-info btn-sm">Consulting/Session Time</button></a><br>';
                            if($value["status"]==0)
                            {   
                                $html.='<button class="btn btn-danger btn-sm" onclick="update_contact_status('.$value["contact_id"].',1);" style="margin-top:5px;">Inactive</button>';
                            }
                            else
                            {
                                $html.='<button class="btn btn-success btn-sm" onclick="update_contact_status('.$value["contact_id"].',0);" style="margin-top:5px;">Active</button>';
                            }
    ...
    {
        $html='<h5>No Contacts Found</h5>';
    }
    echo $html;
}
//add by ample 08-07-20
elseif ($action == 'holiday_delete_data') {
    $data_id = stripslashes($_REQUEST['data_id']);

    echo $obj->holiday_data_delete($data_id);
}
//add by ample 09-07-20
elseif ($action == 'slot_delete_data') {
    $data_id = stripslashes($_REQUEST['data_id']);

    echo $obj->slot_data_delete($data_id);
}
//add by ample 17-04-20
elseif ($action == 'vendorContactUpdateStatus') {
    $contact_id = stripslashes($_REQUEST['contact_id']);
    $status = stripslashes($_REQUEST['status']); 

    $res=$obj->contact_status_update($contact_id, $status);

    if($res==true)
    {
        echo 'Status update succcessfully!';
    }
    else
    {
        echo 'Try Later!';
...
{   
    ...
    {
        echo $obj->get_user_name_by_referenceID($code);
    }
