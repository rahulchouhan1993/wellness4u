...
$obj->debuglog('[REGISTER] REQUEST:<pre>'.print_r($_REQUEST,true).'</pre>, FILES:<pre>'.print_r($_FILES,true).'</pre>');
...
//                echo '</pre>';
...

	{
		...
		$response = array('msg'=>'Please enter valid password','status'=>0);
	...
	elseif(!$obj->chkValidPassword($vendor_password))

	{
		...
		$response = array('msg'=>'Please enter valid Password.<br>Atleast 1 Upper case alphabate[A-Z],<br> 1 Lower case alphabate[a-z] ,<br> 1 Numeric[0-9] ,<br>  1 special characters[!@#$%^&*()-_=+,<>./?]','status'=>0);
		...
		exit(0);
        	...
        	if($obj->chkVendorEmailExists($vendor_email))
            {
           	...
           	if($obj->chkVendorMobileExists($vendor_mobile))
            {

                    $error = true;

                    $err_msg = 'Vendor mobile already exists';
    	...
    	if(!$obj->chkValidReferenceID($reference_id))
            {
                    ...
                    $err_msg = 'Reference ID is not valid!';
...
//			}
...

	{
		...
		$tdata['reference_id'] = $reference_id; //added by ample 30-11-20
...

		{
                       ...
                       $obj->updatevendoruniqueid($vendor_unique_id,$vendor_id);
                         ...
                         $em=$obj->sendSignUpEmailToVendor($tdata['vendor_email'],$tdata['vendor_mobile']);


                    if(!empty($reference_id))
					{
						$obj->update_advice_referal_status($vendor_email,$reference_id);
					}
                        

			$msg = 'Record Added Successfully!';

			// $ref_url = "https://www.wellnessway4u.com/messages.php?id=5&sess=".base64_encode($tdata['vendor_email']);



			$ref_url = "https://www.wellnessway4u.com/message.php?msg=12&sess=".base64_encode($tdata['vendor_email']);
