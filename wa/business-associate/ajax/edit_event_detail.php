...

{
       ...
       //add by ample
	$event_master_id = strip_tags(trim($_POST['event_master_id']));
	...
	if(isset($_POST['online_registration_link']))

	{

		foreach($_POST['online_registration_link'] as $key => $val)

		{

			array_push($arr_online_registration_link,$val);
	...
	if(isset($_POST['online_registration_id']))

	{

		foreach($_POST['online_registration_id'] as $key => $val)

		{

			array_push($arr_online_registration_id,$val);
 ...
 //    $arr_participants_profile = array();
	...
	// if(isset($_POST['participants_profile']))

	// {

	// 	foreach($_POST['participants_profile'] as $key => $val)

	// 	{

	// 		array_push($arr_participants_profile ,$val);
...
//		}
...

	{
...

		{
...

			{
				...
				$arr_vloc_doc_file[$i] = $_POST['hdnvloc_doc_file'][$i];
...

			{
				...
				$arr_vloc_menu_file[$i] = $_POST['hdnvloc_menu_file'][$i];
...

			{
				...
				$arr_venue_image_file[$i] = $_POST['hdnvenue_image_file'][$i];
...

			{
				...
				$arr_event_image_file[$i] = $_POST['hdnevent_image_file'][$i];	
...

	{
		...
		$tdata['event_master_id'] =$event_master_id; //add by ample

		$tdata['event_id'] =$event_id; //add by ample
		...
		$tdata['area_id'] = $arr_area_id;
                ...
                $tdata['online_registration_link'] = $arr_online_registration_link;
                $tdata['online_registration_id'] = $arr_online_registration_id;

                //add by ample 20-12-19
                // $tdata['participants_profile'] = $arr_participants_profile;
		...
		if($obj->updateEventLocInfo($tdata))

		{

			$msg = 'Record Updated Successfully!';

			$ref_url = "edit_events.php?token=".base64_encode($event_master_id)."&msg=".urlencode($msg);
