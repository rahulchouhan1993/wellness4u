...
function doCheckoutSaveDeliveryAddress()
{
	...
	{
		$('#err_msg_delivery').html('<p class="err_msg">Please enter location for delivery address.</p>');
		error = true;
	}
	...
	{
		$('#err_msg_delivery').html('<p class="err_msg">Please enter location for billing address.</p>');
		error = true;
	}
	else if(delivery_mobile_no == '')
	{
	...
	if(!error)
	{
		var dataString = 'action=docheckoutsavedeliveryaddress&delivery_building_name='+escape(delivery_building_name)+'&delivery_floor_no='+escape(delivery_floor_no)+'&delivery_address_line1='+escape(delivery_address_line1)+'&delivery_landmark='+escape(delivery_landmark)+'&delivery_city_id='+escape(delivery_city_id)+'&delivery_area_id='+escape(delivery_area_id)+'&delivery_mobile_no='+escape(delivery_mobile_no)+'&delivery_pincode='+escape(delivery_pincode)+'&billing_building_name='+escape(billing_building_name)+'&billing_floor_no='+escape(billing_floor_no)+'&billing_address_line1='+escape(billing_address_line1)+'&billing_landmark='+escape(billing_landmark)+'&billing_city_id='+escape(billing_city_id)+'&billing_area_id='+escape(billing_area_id)+'&billing_mobile_no='+escape(billing_mobile_no)+'&billing_pincode='+escape(billing_pincode);
		$.ajax({
			type: "POST",
			url: "remote.php",
			data: dataString,
