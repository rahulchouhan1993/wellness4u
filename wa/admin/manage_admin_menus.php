...
{ 
	...
	if($obj->chkIfAccessOfMenuAction($admin_id,$view_action_id))
	{ ?>
    var dataString ='action=adminmenulist';
	$.ajax({
		type: "POST",
		url: "ajax/remote.php",
		data: dataString,
...
{ ?>	
function deleteMultipleAdminMenu()
{
	var favorite = [];
	var chkbox_records = "";
	...
	else
	{
		...
		if (r == true) 
		{
			var dataString ='chkbox_records='+chkbox_records+'&action=deletemultipleadminmenu';
			$.ajax({
				type: "POST",
				url: "ajax/remote.php",
				data: dataString,
