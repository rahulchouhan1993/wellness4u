...
{
	$sp_id = base64_decode($_GET['token']);
	$arr_record = $obj->getShippingPriceDetails($sp_id);
	if(count($arr_record) == 0)
	{
		header("Location: manage_shipping_prices.php");
		exit(0);
	}
	
	$shipping_price = $arr_record['shipping_price'];
	$min_order_amount = $arr_record['min_order_amount']; 
	$max_order_amount = $arr_record['max_order_amount']; 
	$sp_type = $arr_record['sp_type']; 
	$sp_percentage = $arr_record['sp_percentage']; 
	$sp_min_qty_id = $arr_record['sp_min_qty_id']; 
	$sp_min_qty_val = $arr_record['sp_min_qty_val']; 
	$sp_max_qty_id = $arr_record['sp_max_qty_id']; 
	$sp_max_qty_val = $arr_record['sp_max_qty_val']; 
	$sp_applied_on = $arr_record['sp_applied_on']; 
	$sp_effective_date = $arr_record['sp_effective_date']; 
	$sp_comments = $arr_record['sp_comments']; 
	
	$sp_effective_date = date('d-m-Y',strtotime($sp_effective_date));
	
	$sp_country_id = $arr_record['sp_country_id'];
	if($sp_country_id == '-1' || $sp_country_id == '')
	{
		$arr_country_id = array('-1');	
	}
	else
	{
		$arr_country_id = explode(',',$sp_country_id);	
	}
	
	$sp_state_id = $arr_record['sp_state_id'];
	if($sp_state_id == '-1' || $sp_state_id == '')
	{
		$arr_state_id = array('-1');	
	}
	else
	{
		$arr_state_id = explode(',',$sp_state_id);	
	}
	
	$sp_city_id = $arr_record['sp_city_id'];
	if($sp_city_id == '-1' || $sp_city_id == '')
	{
		$arr_city_id = array('-1');	
	}
	else
	{
		$arr_city_id = explode(',',$sp_city_id);	
	}
	
	$sp_area_id = $arr_record['sp_area_id'];
	if($sp_area_id == '-1' || $sp_area_id == '')
	{
		$arr_area_id = array('-1');	
	}
	else
	{
		$arr_area_id = explode(',',$sp_area_id);	
	}
	
	$sp_status = $arr_record['sp_status'];
		...
		<div class="col-md-12">
			<div class="panel">
				<div class="panel-body">
					...
					<form role="form" class="form-horizontal" id="edit_shipping_price" name="edit_shipping_price" method="post"> 
						<input type="hidden" name="hdnsp_id" id="hdnsp_id" value="<?php echo $sp_id;?>" >
						...
						<div class="form-group">
							...
							<div class="col-lg-4">
								<select name="sp_status" id="sp_status" class="form-control">
									<option value="1" <?php if($sp_status == '1'){?> selected <?php } ?>>Active</option> 
									<option value="0" <?php if($sp_status == '0'){?> selected <?php } ?>>Inactive</option> 
								</select>
							</div>
						</div>
...
<script src="admin-js/edit-shipping-price-validator.js" type="text/javascript"></script>
<script>
