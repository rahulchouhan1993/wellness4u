...
{ 
	...
	if($obj->chkIfAccessOfMenuAction($admin_id,$view_action_id))
	{ ?>
    var dataString ='action=citylist';
	$.ajax({
		type: "POST",
		url: "ajax/remote.php",
		data: dataString,
...
function change_page(page_id)  
{
	...
	var dataString ='page_id='+page_id +'&txtsearch='+escape(txtsearch)+'&status='+status+'&country_id='+country_id+'&state_id='+state_id+'&added_by_admin='+escape(added_by_admin)+'&added_date_type='+escape(added_date_type)+'&added_days_of_month='+escape(added_days_of_month)+'&added_days_of_week='+escape(added_days_of_week)+'&added_single_date='+escape(added_single_date)+'&added_start_date='+escape(added_start_date)+'&added_end_date='+escape(added_end_date)+'&action=citylist';
	$.ajax({
		type: "POST",
		url: "ajax/remote.php",
		data: dataString,
...
function deletemultiplecities()
{
	...
	else
	{
		...
		if (r == true) 
		{
			var dataString ='chkbox_records='+chkbox_records+'&action=deletemultiplecities';
			$.ajax({
				type: "POST",
				url: "ajax/remote.php",
				data: dataString,
