...
if(isset($_GET['token']) && $_GET['token'] != '' && isset($_GET['vendor']) && $_GET['vendor'] != '')

{
	...
	$arr_loc_record = $obj->getVendorAllLocationsAndCertifications($vendor_id,$vendor_location_id);
	...
	if(isset($_POST['staff_save']) && !empty($_POST) )
	{
		// 	echo '<br><pre>';
		// print_r($_POST);
		// echo '<br></pre>';
		...
		$data=array();
		$vloc_id=$_POST['staff_vloc'];
		$count_row=count($_POST['contact_person_title']);
		for ($i=0; $i < $count_row; $i++) { 
			$data[] = array( 'contact_person_title' =>$_POST['contact_person_title'][$i] ,
						   	'contact_person' =>$_POST['contact_person'][$i] ,
						   	'contact_email' =>$_POST['contact_email'][$i] ,
						   	'contact_number' =>$_POST['contact_number'][$i] ,
						   	'contact_designation' =>$_POST['contact_designation'][$i] ,
						   	'contact_remark' =>$_POST['contact_remark'][$i] ,
						   	'speciality_offered' =>implode(',', $_POST['speciality_offered'][$i]) ,
					);	
		}
		$res=$obj->save_vendor_contacts($vendor_id,$vloc_id,$data);
		if($res)
		{	
			$_SESSION['MSG'] = 'Contacts save Successfully!';
			header("Location: edit_location_contact.php?token=".$_GET['token']);
			exit(0);
		}
		...
		<div class="col-md-12">
			...
			<div class="panel">
				...
				<div class="panel-body">
					...
					<form role="form" class="form-horizontal" id="edit_vendor_loc" name="edit_vendor_loc" method="post"> 
					...
					{ ?>
						...
						<div id="row_loc_<?php if($i == 0){ echo 'first';}else{ echo $i;}?>" style="border:1px solid #CCC;padding:5px;margin: 10px 0px;">
							...
							<button type="button" onclick="add_staff('<?php echo $arr_vloc_id[$i];?>')">Add Work Force</button> <button type="button" onclick="view_staff('<?php echo $arr_vloc_id[$i];?>')">Update Professional details</button>
  ...
  <div class="modal-dialog modal-lg">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title">Add Work Force</h4>
      </div>
      <div class="modal-body">
        <form class="form-horizontal" method="post" action="">
        	<div id="staff_box_form">
        	<input type="hidden" name="box_count" value="0" id="box_count">
        	<div class="box" id="box0">
	        	<div class="form-group">
					<label class="col-lg-2 control-label">Gender<span style="color:red">*</span></label>
					<div class="col-lg-4">
						<select name="contact_person_title[]" id="contact_person_title_0" class="form-control" required>
							<?php echo $obj->getPersonTitleOption('');?>
						</select>
					</div>
					<label class="col-lg-2 control-label">Contact Person<span style="color:red">*</span></label>
					<div class="col-lg-4">
						<input type="text" name="contact_person[]" id="contact_person_0" value="" placeholder="Contact Person" class="form-control" required>
					</div>
				</div>
				<div class="form-group">
					<label class="col-lg-2 control-label">Contact Email<span style="color:red">*</span></label>
					<div class="col-lg-4">
						<input type="text" name="contact_email[]" id="contact_email_0" value="" placeholder="Contact Email" class="form-control" required>
					</div>
					<label class="col-lg-2 control-label">Contact Number<span style="color:red">*</span></label>
					<div class="col-lg-4">
						<input type="text" name="contact_number[]" id="contact_number_0" value="" placeholder="Contact Number" class="form-control" required>
					</div>
				</div>
				<div class="form-group">
					...
					<label class="col-lg-2 control-label">Remark</label>
					<div class="col-lg-4">
		                 <textarea name="contact_remark[]" id="contact_remark_0"  class="form-control"></textarea>
					</div>
				</div>
				<div class="form-group">
					<label class="col-lg-2 control-label">Speciality Offered</label>
					<div class="col-lg-10">
						<select name="speciality_offered[0][]" id="vloc_speciality_offered_0" multiple="multiple" class="form-control vloc_speciality_offered" >
		                  <?php echo $obj->getFavCategoryVendorEdit('13,53,42,64',''); ?>
						</select>
					</div>
				</div>
				<button type="button" class="btn btn-success btn-sm" onclick="add_staff_box()">Add More</button>
			</div>
			</div>
			<br>
			<input type="hidden" name="staff_vloc" id="staff_vloc">
			<button type="submit" name="staff_save" class="btn btn-primary">Save</button>
        </form>
      </div>
...
<div id="viewModal" class="modal fade" role="dialog">
  <div class="modal-dialog modal-lg">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title">Update Professional details</h4>
      </div>
      <div class="modal-body">
      	 <div id="view_staff_list"></div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
      </div>
...
<script src="admin-js/edit-vendor-location-validator.js" type="text/javascript"></script>
