...
if(isset($_GET['pgid']) && $_GET['pgid'] != '')
{
	$page_id = $_GET['pgid'];
	$arr_record = $obj->getPageDetails($page_id);
	...
	$page_name = $arr_record['page_name'];
	$page_title = $arr_record['page_title'];
	$page_contents = stripslashes($arr_record['page_contents']);
	$meta_title = $arr_record['meta_title'];
	$meta_keywords = $arr_record['meta_keywords']; 
	$meta_desc = $arr_record['meta_desc'];
	$show_in_manage_menu = $arr_record['show_in_manage_menu'];
	$page_link = $arr_record['page_link'];
	$page_status = $arr_record['page_status'];
}	
else
{
	header("Location: manage_pages.php");
		...
		<div class="col-md-12">
			<div class="panel">
				<div class="panel-body">
					...
					<form role="form" class="form-horizontal" id="edit_page" name="edit_page" method="post"> 
						<input type="hidden" name="hdnpage_id" id="hdnpage_id" value="<?php echo $page_id;?>" >
						...
						<div class="form-group">
							...
							<div class="col-lg-5">
								<select name="page_status" id="page_status" class="form-control">
									<option value="1" <?php if($page_status == '1'){?> selected <?php } ?>>Active</option> 
									<option value="0" <?php if($page_status == '0'){?> selected <?php } ?>>Inactive</option> 
								</select>
							</div>
						</div>	
						<hr>
...
<script src="admin-js/edit-page-validator.js" type="text/javascript"></script>
<script>
