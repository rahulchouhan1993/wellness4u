		...
		<div class="col-md-12">
			<div class="panel">
				...
				<div class="search-panel">
					<div class="row" >
						<div class="col-sm-2"><strong>Search:</strong>&nbsp;<br>
							<input type="text" id="txtsearch" name="txtsearch" value="<?php echo $txtsearch;?>" style="width:150px;" >
						</div>
						...
						<div class="col-sm-2"><strong>Ingredients:</strong>&nbsp;<br>
							<select name="ingredient_id" id="ingredient_id"  class="form-control" >
								<option value="">Select Ingredients</option>
								<?php echo $obj->GetIngredientsById($ingredient_id); ?>
							</select>
						</div>
						<div class="col-sm-2"><strong>Parent Category:</strong>&nbsp;<br>
							<select name="parent_id" id="parent_id"  class="form-control" onchange="getsubcat();">
								<?php echo $obj->GetCategories($parent_id,'0'); ?>
							</select>
						</div>
						<div class="col-sm-2"><strong>Main Category:</strong>&nbsp;<br>
...
{ 
	...
	if($obj->chkIfAccessOfMenuAction($admin_id,$view_action_id))
	{ ?>
    var dataString ='action=itemslist';
	$.ajax({
		type: "POST",
		url: "ajax/remote.php",
		data: dataString,
...
function change_page(page_id)  
{
	...
	var parent_cat_id = $("#parent_id").val();
	var main_cat_id = $("#main_id").val();
	...
	var dataString ='page_id='+page_id +'&txtsearch='+escape(txtsearch)+'&status='+status+'&ingredient_id='+ingredient_id+'&parent_cat_id='+parent_cat_id+'&main_cat_id='+main_cat_id+'&added_by_admin='+escape(added_by_admin)+'&added_date_type='+escape(added_date_type)+'&added_days_of_month='+escape(added_days_of_month)+'&added_days_of_week='+escape(added_days_of_week)+'&added_single_date='+escape(added_single_date)+'&added_start_date='+escape(added_start_date)+'&added_end_date='+escape(added_end_date)+'&action=itemslist';
	$.ajax({
		type: "POST",
		url: "ajax/remote.php",
		data: dataString,
...
function refineSearch()  
{
	...
	var dataString ='txtsearch='+escape(txtsearch)+'&status='+status+'&ingredient_id='+ingredient_id+'&parent_cat_id='+parent_cat_id+'&main_cat_id='+main_cat_id+'&added_by_admin='+escape(added_by_admin)+'&added_date_type='+escape(added_date_type)+'&added_days_of_month='+escape(added_days_of_month)+'&added_days_of_week='+escape(added_days_of_week)+'&added_single_date='+escape(added_single_date)+'&added_start_date='+escape(added_start_date)+'&added_end_date='+escape(added_end_date)+'&action=itemslist';
	$.ajax({
		type: "POST",
		url: "ajax/remote.php",
		data: dataString,
...
function deletemultipleitems()
{
	...
	else
	{
		...
		if (r == true) 
		{
			var dataString ='chkbox_records='+chkbox_records+'&action=deletemultipleitems';
