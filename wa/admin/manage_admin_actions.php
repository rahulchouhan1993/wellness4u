		...
		<div class="col-md-12">
			<div class="panel">
				...
				<header class="panel-heading">
					<h2 class="panel-title">Manage Admin Actions - Menu (<?php echo $arr_record['am_title'];?>)</h2>
				</header>
				<div class="pull-right tooltip-show">
				<a href="manage_admin_menus.php" class="btn btn-default btn-sm" data-toggle="tooltip" data-placement="top" title="Back to Parent Menu" data-original-title=""><i class="fa fa-backward"></i></a>
				<?php
				if($obj->chkIfAccessOfMenuAction($admin_id,$add_action_id))
				{ ?>
					<a href="add_admin_action.php?am_id=<?php echo $am_id;?>" class="btn btn-default btn-sm" data-toggle="tooltip" data-placement="top" title="Add New Admin Action" data-original-title=""><i class="fa fa-plus"></i></a>
				<?php
				} ?>	
				</div>
...
{ 
	...
	{ ?>
	var am_id = $("#am_id").val();
    var dataString ='am_id='+escape(am_id) +'&action=adminactionlist';
	$.ajax({
		type: "POST",
		url: "ajax/remote.php",
		data: dataString,
...
function change_page(page_id)  
{
	...
	var dataString ='am_id='+escape(am_id) +'&page_id='+page_id +'&txtsearch='+escape(txtsearch) +'&status='+status+'&added_by_admin='+escape(added_by_admin)+'&added_date_type='+escape(added_date_type)+'&added_days_of_month='+escape(added_days_of_month)+'&added_days_of_week='+escape(added_days_of_week)+'&added_single_date='+escape(added_single_date)+'&added_start_date='+escape(added_start_date)+'&added_end_date='+escape(added_end_date)+'&action=adminactionlist';
	$.ajax({
		type: "POST",
		url: "ajax/remote.php",
		data: dataString,
...
function refineSearch()  
{
	...
	var dataString ='txtsearch='+escape(txtsearch) +'&am_id='+escape(am_id) +'&status='+status+'&added_by_admin='+escape(added_by_admin)+'&added_date_type='+escape(added_date_type)+'&added_days_of_month='+escape(added_days_of_month)+'&added_days_of_week='+escape(added_days_of_week)+'&added_single_date='+escape(added_single_date)+'&added_start_date='+escape(added_start_date)+'&added_end_date='+escape(added_end_date)+'&action=adminactionlist';
	$.ajax({
		type: "POST",
		url: "ajax/remote.php",
		data: dataString,
...
function deleteMultipleAdminAction()
{
	var favorite = [];
	var chkbox_records = "";
	...
	else
	{
		...
		if (r == true) 
		{
			var dataString ='chkbox_records='+chkbox_records+'&action=deletemultipleadminaction';
			$.ajax({
				type: "POST",
				url: "ajax/remote.php",
				data: dataString,
