		...
		<div class="col-md-12">
			<div class="panel">
				...
				<div class="search-panel">
					<div class="row" >
						...
						<div class="col-sm-3"><strong>Status:</strong>&nbsp;<br>
							<select name="delivery_status" id="delivery_status" class="form-control">
								<?php echo $obj->getOrderDeiveryStatusOption($delivery_status,2); ?>
							</select>
						</div>
					...
					<div class="row" >
						<div class="col-sm-3"><strong>Logistic Partner Type:</strong>&nbsp;<br>
							<select name="logistic_partner_type_cat_id" id="logistic_partner_type_cat_id" onchange="getLogisticPartnerOption('2')" class="form-control" required>
								<?php echo $obj->getMainCategoryOption($logistic_partner_type_parent_cat_id,$logistic_partner_type_cat_id,2); ?>
							</select>
						</div>
						<div class="col-sm-3"><strong>Logistic Partner:</strong>&nbsp;<br>
							<select name="logistic_partner_id" id="logistic_partner_id" class="form-control">
								<?php echo $obj->getLogisticPartnerOption($logistic_partner_type_cat_id,$logistic_partner_id,2); ?>
							</select>
						</div>
					</div>
					<div class="row">
...
{ 
	...
	if($obj->chkIfAccessOfMenuAction($admin_id,$view_action_id))
	{ ?>
    var dataString ='action=orderdeliverylist';
	$.ajax({
		type: "POST",
		url: "ajax/remote.php",
		data: dataString,
...
function change_page(page_id)  
{
	...
	var logistic_partner_type_cat_id = $("#logistic_partner_type_cat_id").val();
	var logistic_partner_id = $(logistic_partner_id).val();
	...
	var dataString ='page_id='+page_id +'&txtsearch='+escape(txtsearch)+'&delivery_status='+delivery_status+'&delivery_date='+delivery_date+'&logistic_partner_type_cat_id='+logistic_partner_type_cat_id+'&logistic_partner_id='+logistic_partner_id+'&added_by_admin='+escape(added_by_admin)+'&added_date_type='+escape(added_date_type)+'&added_days_of_month='+escape(added_days_of_month)+'&added_days_of_week='+escape(added_days_of_week)+'&added_single_date='+escape(added_single_date)+'&added_start_date='+escape(added_start_date)+'&added_end_date='+escape(added_end_date)+'&action=orderdeliverylist';
	$.ajax({
		type: "POST",
		url: "ajax/remote.php",
		data: dataString,
...
function refineSearch()  
{
	...
	var dataString ='txtsearch='+escape(txtsearch)+'&delivery_status='+delivery_status+'&delivery_date='+delivery_date+'&logistic_partner_type_cat_id='+logistic_partner_type_cat_id+'&logistic_partner_id='+logistic_partner_id+'&added_by_admin='+escape(added_by_admin)+'&added_date_type='+escape(added_date_type)+'&added_days_of_month='+escape(added_days_of_month)+'&added_days_of_week='+escape(added_days_of_week)+'&added_single_date='+escape(added_single_date)+'&added_start_date='+escape(added_start_date)+'&added_end_date='+escape(added_end_date)+'&action=orderdeliverylist';
	$.ajax({
		type: "POST",
		url: "ajax/remote.php",
		data: dataString,
...
{ ?>	
function deletemultipleorderdelivery()
{
	var favorite = [];
	var chkbox_records = "";
	...
	else
	{
		...
		if (r == true) 
		{
			var dataString ='chkbox_records='+chkbox_records+'&action=deletemultipleorderdelivery';
