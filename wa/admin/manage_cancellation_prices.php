			...
			<div class="panel">
				...
				<div class="search-panel">
					<div class="row" >
						...
						<div class="col-sm-3"><strong>Cancellation Type:</strong>&nbsp;<br>
							<select name="cp_type" id="cp_type" onchange="getShippingAppliedOnOption('2');" class="form-control">
								<?php echo $obj->getCancellationTypeOption($cp_type,2); ?>
							</select>
						</div>
						<div class="col-sm-3"><strong>Cancellation Applied On:</strong>&nbsp;<br>
							<select name="cp_applied_on" id="cp_applied_on" class="form-control">
								<?php echo $obj->getCancellationAppliedOnOption($cp_type,$cp_applied_on,2); ?>
							</select>
						</div>
					</div>
					<div class="row">
					...
					<div class="row" >	
						<div class="col-sm-3">
							...
							<input type="text" name="cp_effective_date" id="cp_effective_date" placeholder="Select Effective Date" class="form-control">
						</div>
						<div class="col-sm-3">	
				...
				<div class="panel-body">
					...
					if($obj->chkIfAccessOfMenuAction($admin_id,$view_action_id))
					{ ?>
					<div id="cancellationpriceslist"></div>
					<?php
					} ?>
				</div>
			</div>
...
{ 
	...
	if($obj->chkIfAccessOfMenuAction($admin_id,$view_action_id))
	{ ?>
    var dataString ='action=cancellationpriceslist';
	$.ajax({
		type: "POST",
		url: "ajax/remote.php",
		data: dataString,
		...
		{
			$("#cancellationpriceslist").html(result);
		}
	});
	<?php
	} ?>
...
function change_page(page_id)  
{
	...
	var cp_effective_date = $("#cp_effective_date").val();
	
	var dataString ='page_id='+page_id +'&txtsearch='+escape(txtsearch)+'&status='+status+'&cp_type='+cp_type+'&cp_applied_on='+cp_applied_on+'&added_by_admin='+escape(added_by_admin)+'&added_date_type='+escape(added_date_type)+'&added_days_of_month='+escape(added_days_of_month)+'&added_days_of_week='+escape(added_days_of_week)+'&added_single_date='+escape(added_single_date)+'&added_start_date='+escape(added_start_date)+'&added_end_date='+escape(added_end_date)+'&cp_effective_date='+escape(cp_effective_date)+'&action=cancellationpriceslist';
	$.ajax({
		type: "POST",
		url: "ajax/remote.php",
		data: dataString,
...
function refineSearch()  
{
	...
	var dataString ='txtsearch='+escape(txtsearch)+'&status='+status+'&cp_type='+cp_type+'&cp_applied_on='+cp_applied_on+'&added_by_admin='+escape(added_by_admin)+'&added_date_type='+escape(added_date_type)+'&added_days_of_month='+escape(added_days_of_month)+'&added_days_of_week='+escape(added_days_of_week)+'&added_single_date='+escape(added_single_date)+'&added_start_date='+escape(added_start_date)+'&added_end_date='+escape(added_end_date)+'&cp_effective_date='+escape(cp_effective_date)+'&action=cancellationpriceslist';
	$.ajax({
		type: "POST",
		url: "ajax/remote.php",
		data: dataString,
...
{ ?>	
function deletemultiplecancellationprices()
{
	var favorite = [];
	var chkbox_records = "";
	...
	else
	{
		...
		if (r == true) 
		{
			var dataString ='chkbox_records='+chkbox_records+'&action=deletemultiplecancellationprices';
