...
if(isset($_POST['btnSubmit']))
{
	...
	$food_products_offered=implode(',', $_POST['food_products_offered']); // add by ample 18-09-20
	...
	$arr_hdnvloc_doc_file = array();
	if(isset($_POST['hdnvloc_doc_file']))
	{
		foreach($_POST['hdnvloc_doc_file'] as $key => $val)
		{
			array_push($arr_hdnvloc_doc_file,$val);
		}		
	}
	
	$arr_hdnvloc_menu_file = array();
	if(isset($_POST['hdnvloc_menu_file']))
	{
		foreach($_POST['hdnvloc_menu_file'] as $key => $val)
		{
			array_push($arr_hdnvloc_menu_file,$val);
		}		
	}
	...
	for($i=0;$i<count($arr_cert_loop_cnt);$i++)
	{
		...
		if(isset($_POST['vc_cert_id_'.$arr_cert_loop_cnt[$i]]))
		{
			foreach($_POST['vc_cert_id_'.$arr_cert_loop_cnt[$i]] as $key => $val)
			{
				array_push($arr_vc_cert_id[$i],$val);
			}		
		}
	...
	for($i=0,$j=1;$i<count($arr_vloc_parent_cat_id);$i++,$j++)
	{
		...
		if($i == 0)
		{
			...
			elseif($obj->chkVendorEmailExists_edit($arr_contact_email[$i],$vendor_id))
			{
				$error = true;
	...
	if(!$error)
	{
		...
		for($i=0,$j=1;$i<count($arr_vloc_parent_cat_id);$i++,$j++)
		{
			...
			{
				$arr_vloc_doc_file[$i] = $arr_hdnvloc_doc_file[$i];		
			}
			...
			{
				$arr_vloc_menu_file[$i] = $arr_hdnvloc_menu_file[$i];		
			}
		}
	}
	...
	if(!$error)
	{
		...
		for($i=0;$i<count($arr_cert_loop_cnt);$i++)
		{
			...
			for($k=0,$l=1;$k<count($arr_vc_cert_type_id[$i]);$k++,$l++)
			{
				...
				{
					if(isset($_POST['hdnvc_cert_scan_file_'.$arr_cert_loop_cnt[$i].'_'.$k]))
					{
						$arr_vc_cert_scan_file[$i][$k] = $_POST['hdnvc_cert_scan_file_'.$arr_cert_loop_cnt[$i].'_'.$k];		
					}
					else
					{
	...
	if(!$error)
	{
        ...
        $tdata['vendor_mobile'] = $vendor_mobile; // update by ample 18-02-20
		$tdata['vendor_email'] = $vendor_email;  // update by ample 18-02-20
		$tdata['food_products_offered'] = $food_products_offered; // update by ample 18-02-20
		...
		$tdata['vc_cert_id'] = $arr_vc_cert_id;
		...
		if($obj->chkIfNewVendor($vendor_id))
		{
			$flag_new_vendor = true;
		}
		else
		{
			$flag_new_vendor = false;
		}
		
		if($obj->updateVendor($tdata))
		{
			if($flag_new_vendor)
			{
				//$obj->debuglog('[edit_vendor] in flag_new_vendor, status:'.$tdata['vendor_status'].';');
				if($tdata['vendor_status'] == '1')
				{
					//$obj->debuglog('[edit_vendor] in vendor_status, status:'.$tdata['vendor_status'].';');
					$arr_where_new = array();
					$arr_where_new['vendor_id'] = $vendor_id;
					//$arr_where_new['new_vendor'] = '1';
					
					$tdata_new = array();
					$tdata_new['tablename'] = 'tblvendors';
					$tdata_new['new_vendor'] = '0';
					$tdata_new['vendor_modified_date'] = date('Y-m-d H:i:s');
					$tdata_new['modified_by_admin'] = $admin_id;
					
					$obj->updateRecordCommon($tdata_new,$arr_where_new,1);
					$obj->sendProfileActivationEmailToVendor($tdata['vendor_email']);	
				}
			}
