...
$obj->debuglog('[ADD_BANNER_SLIDER] REQUEST:<pre>'.print_r($_REQUEST,true).'</pre>, FILES:<pre>'.print_r($_FILES,true).'</pre>');
...
if(isset($_POST['btnSubmit']))
{
	...
	if(!$error)
	{
		...
		if (!empty($_FILES) && ($_FILES['banner_image']['tmp_name'] != '') )
		{
	...
	if(!$error)
	{
		...
		$tdata['banner_status'] = 1;
		$tdata['added_by_admin'] = $admin_id;
		
		if($obj->addBannerSlider($tdata))
		{
			$msg = 'Record added successfully!';
			$ref_url = "manage_banner_siliders.php?msg=".urlencode($msg);
