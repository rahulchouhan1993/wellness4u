		...
		<div class="col-md-12">
			<div class="panel">
				...
				<header class="panel-heading">
					<h2 class="panel-title">Manage Forms - Menu (<?php echo $arr_record['va_name'];?>) For Category: <?php echo $va_cat_name;?> > <?php echo $va_sub_cat_name; ?></h2>
				</header>
				<div class="pull-right tooltip-show">
				<a href="manage_vendors_access.php" class="btn btn-default btn-sm" data-toggle="tooltip" data-placement="top" title="Back to Parent Menu" data-original-title=""><i class="fa fa-backward"></i></a>
				<?php
				if($obj->chkIfAccessOfMenuAction($admin_id,$add_action_id))
				{ ?>
					<a href="<?php echo $obj->getAdminActionLink($add_action_id);?>?va_id=<?php echo $va_id;?>" class="btn btn-default btn-sm" data-toggle="tooltip" data-placement="top" title="<?php echo $obj->getAdminActionName($add_action_id);?>" data-original-title=""><i class="fa fa-plus"></i></a>
				<?php
				} ?>	
				</div>
				...
				<div class="panel-body">
					...
					if($obj->chkIfAccessOfMenuAction($admin_id,$view_action_id))
					{ ?>
					<div id="vendorsaccessformslist"></div>
					<?php
					} ?>
				</div>
			</div>
...
{ 
	...
	{ ?>
	var va_id = $("#va_id").val();
    var dataString ='va_id='+escape(va_id) +'&action=vendorsaccessformslist';
	$.ajax({
		type: "POST",
		url: "ajax/remote.php",
		data: dataString,
		...
		{
			$("#vendorsaccessformslist").html(result);
		}
	});
	<?php
	} ?>
...
function change_page(page_id)  
{
	...
	var dataString ='va_id='+escape(va_id) +'&page_id='+page_id +'&txtsearch='+escape(txtsearch) +'&status='+status+'&added_by_admin='+escape(added_by_admin)+'&added_date_type='+escape(added_date_type)+'&added_days_of_month='+escape(added_days_of_month)+'&added_days_of_week='+escape(added_days_of_week)+'&added_single_date='+escape(added_single_date)+'&added_start_date='+escape(added_start_date)+'&added_end_date='+escape(added_end_date)+'&action=vendorsaccessformslist';
	$.ajax({
		type: "POST",
		url: "ajax/remote.php",
		data: dataString,
...
function refineSearch()  
{
	...
	var dataString ='txtsearch='+escape(txtsearch) +'&va_id='+escape(va_id) +'&status='+status+'&added_by_admin='+escape(added_by_admin)+'&added_date_type='+escape(added_date_type)+'&added_days_of_month='+escape(added_days_of_month)+'&added_days_of_week='+escape(added_days_of_week)+'&added_single_date='+escape(added_single_date)+'&added_start_date='+escape(added_start_date)+'&added_end_date='+escape(added_end_date)+'&action=vendorsaccessformslist';
	$.ajax({
		type: "POST",
		url: "ajax/remote.php",
		data: dataString,
...
function deletemultiplevendorsaccessforms()
{
	var favorite = [];
	var chkbox_records = "";
	...
	else
	{
		...
		if (r == true) 
		{
			var dataString ='chkbox_records='+chkbox_records+'&action=deletemultiplevendorsaccessforms';
			$.ajax({
				type: "POST",
				url: "ajax/remote.php",
				data: dataString,
				...
				success: function(result)
				{
					...
					{
						var va_id = $('#va_id').val();
						window.location.href="manage_vendors_access_forms.php?va_id="+va_id;  
					}
					else
					{
						BootstrapDialog.show({
