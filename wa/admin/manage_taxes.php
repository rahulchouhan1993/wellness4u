		...
		<div class="col-md-12">
			<div class="panel">
				...
				<div class="search-panel">
					<div class="row" >
						...
						<div class="col-lg-3">
							...
							<select name="tax_type" id="tax_type" onchange="showHideTaxQtyDropdowns()" class="form-control" required>
								<?php echo $obj->getTaxTypeOption($tax_type,2); ?>
							</select>
						</div>
						<div class="col-lg-3">
							<div id="show_tax_qty_id" style="display:<?php echo $show_tax_qty_id;?>">
								<strong>Tax Quantity Unit:</strong>&nbsp;<br>
								<select name="tax_qty_id" id="tax_qty_id" class="form-control">
									<?php echo $obj->getMainCategoryOption($tax_qty_parent_id,$tax_qty_id,'2'); ?>
								</select>
							</div>	
							<div id="show_tax_amount" style="display:<?php echo $show_tax_amount;?>"></div>
							<div id="show_tax_percentage" style="display:<?php echo $show_tax_percentage;?>"></div>
						</div>	
					</div>
					<div class="row" >
					...
					<div class="row" >
						<div class="col-sm-3"><strong>Effective Date:</strong>&nbsp;<br>
							<input type="text" id="tax_effective_date" name="tax_effective_date" value="<?php echo $tax_effective_date;?>" class="form-control"  >
						</div>
						<div class="col-lg-3">
							<strong>Tax Applied On:</strong>&nbsp;<br>
							<select name="tax_applied_on" id="tax_applied_on" class="form-control" required>
								<?php echo $obj->getTaxAppliedOnOption($tax_applied_on,2); ?>
							</select>
						</div>
					</div>
					<div class="row" >	
...
{ 
	...
	if($obj->chkIfAccessOfMenuAction($admin_id,$view_action_id))
	{ ?>
    var dataString ='action=taxeslist';
	$.ajax({
		type: "POST",
		url: "ajax/remote.php",
		data: dataString,
...
function change_page(page_id)  
{
	...
	var tax_qty_id = $("#tax_qty_id").val();
	var tax_effective_date = $("#tax_effective_date").val();
	var tax_applied_on = $("#tax_applied_on").val();
	
	var dataString ='page_id='+page_id +'&txtsearch='+escape(txtsearch)+'&status='+status+'&country_id='+country_id+'&state_id='+state_id+'&city_id='+city_id+'&area_id='+area_id+'&added_by_admin='+escape(added_by_admin)+'&added_date_type='+escape(added_date_type)+'&added_days_of_month='+escape(added_days_of_month)+'&added_days_of_week='+escape(added_days_of_week)+'&added_single_date='+escape(added_single_date)+'&added_start_date='+escape(added_start_date)+'&added_end_date='+escape(added_end_date)+'&tax_type='+escape(tax_type)+'&tax_qty_id='+escape(tax_qty_id)+'&tax_effective_date='+escape(tax_effective_date)+'&tax_applied_on='+escape(tax_applied_on)+'&action=taxeslist';
	$.ajax({
		type: "POST",
		url: "ajax/remote.php",
		data: dataString,
...
function deletemultipletaxes()
{
	...
	else
	{
		...
		if (r == true) 
		{
			var dataString ='chkbox_records='+chkbox_records+'&action=deletemultipletaxes';
