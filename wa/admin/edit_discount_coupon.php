...
{
	$dc_id = base64_decode($_GET['token']);
	$arr_record = $obj->getDiscountCouponDetails($dc_id);
	if(count($arr_record) == 0)
	{
		header("Location: manage_discount_coupons.php");
		exit(0);
	}
	
	$discount_coupon = $arr_record['discount_coupon'];
	$discount_price = $arr_record['discount_price'];
	$min_order_amount = $arr_record['min_order_amount']; 
	$max_order_amount = $arr_record['max_order_amount']; 
	$dc_type = $arr_record['dc_type']; 
	$dc_percentage = $arr_record['dc_percentage']; 
	$dc_min_qty_id = $arr_record['dc_min_qty_id']; 
	$dc_min_qty_val = $arr_record['dc_min_qty_val']; 
	$dc_max_qty_id = $arr_record['dc_max_qty_id']; 
	$dc_max_qty_val = $arr_record['dc_max_qty_val']; 
	$dc_applied_on = $arr_record['dc_applied_on']; 
	$dc_comments = $arr_record['dc_comments']; 
	$dc_multiuser = $arr_record['dc_multiuser']; 
	$dc_trade_discount = $arr_record['dc_trade_discount']; 
	
	$dc_effective_date_type = $arr_record['dc_effective_date_type'];
	$dc_effective_days_of_month = $arr_record['dc_effective_days_of_month'];
	$dc_effective_days_of_week = $arr_record['dc_effective_days_of_week'];
	$dc_effective_single_date = $arr_record['dc_effective_single_date'];
	$dc_effective_start_date = $arr_record['dc_effective_start_date'];
	$dc_effective_end_date = $arr_record['dc_effective_end_date'];
	
	if($dc_effective_date_type == 'days_of_month')
	{
		if($dc_effective_days_of_month == '-1' || $dc_effective_days_of_month == '')
		{
			$arr_dc_effective_days_of_month = array('-1');	
		}
		else
		{
			$arr_dc_effective_days_of_month = explode(',',$dc_effective_days_of_month);	
		}
		...
		$dc_effective_show_days_of_month = '';
		$dc_effective_show_days_of_week = 'none';
		$dc_effective_show_single_date = 'none';
		$dc_effective_show_start_date = 'none';
		$dc_effective_show_end_date = 'none';
	...
	{
		if($dc_effective_days_of_week == '-1' || $dc_effective_days_of_week == '')
		{
			$arr_dc_effective_days_of_week = array('-1');	
		}
		else
		{
			$arr_dc_effective_days_of_week = explode(',',$dc_effective_days_of_week);	
		}
		...
		$dc_effective_show_days_of_month = 'none';
		$dc_effective_show_days_of_week = '';
		$dc_effective_show_single_date = 'none';
		$dc_effective_show_start_date = 'none';
		$dc_effective_show_end_date = 'none';
	}
	...
	{
		$dc_effective_single_date = date('d-m-Y',strtotime($dc_effective_single_date));
		...
		$dc_effective_show_days_of_month = 'none';
		$dc_effective_show_days_of_week = 'none';
		$dc_effective_show_single_date = '';
		$dc_effective_show_start_date = 'none';
		$dc_effective_show_end_date = 'none';
	}
	elseif($dc_effective_date_type == 'date_range')
	{
		$dc_effective_start_date = date('d-m-Y',strtotime($dc_effective_start_date));
		$dc_effective_end_date = date('d-m-Y',strtotime($dc_effective_end_date));
		...
		$dc_effective_show_days_of_week = 'none';
		$dc_effective_show_single_date = 'none';
		$dc_effective_show_start_date = '';
		$dc_effective_show_end_date = '';
	}
	else
	{
		$arr_dc_effective_days_of_month = array('-1');
	...
	$dc_country_id = $arr_record['dc_country_id'];
	if($dc_country_id == '-1' || $dc_country_id == '')
	{
		$arr_country_id = array('-1');	
	}
	else
	{
		$arr_country_id = explode(',',$dc_country_id);	
	}
	
	$dc_state_id = $arr_record['dc_state_id'];
	if($dc_state_id == '-1' || $dc_state_id == '')
	{
		$arr_state_id = array('-1');	
	}
	else
	{
		$arr_state_id = explode(',',$dc_state_id);	
	}
	
	$dc_city_id = $arr_record['dc_city_id'];
	if($dc_city_id == '-1' || $dc_city_id == '')
	{
		$arr_city_id = array('-1');	
	}
	else
	{
		$arr_city_id = explode(',',$dc_city_id);	
	}
	
	$dc_area_id = $arr_record['dc_area_id'];
	if($dc_area_id == '-1' || $dc_area_id == '')
	{
		$arr_area_id = array('-1');	
	}
	else
	{
		$arr_area_id = explode(',',$dc_area_id);	
	}
	
	$dc_status = $arr_record['dc_status'];
		...
		<div class="col-md-12">
			<div class="panel">
				<div class="panel-body">
					...
					<form role="form" class="form-horizontal" id="edit_discount_coupon" name="edit_discount_coupon" method="post"> 
						<input type="hidden" name="hdndc_id" id="hdndc_id" value="<?php echo $dc_id;?>" >
						...
						<div class="form-group">
							<label class="col-lg-2 control-label"><strong>Effective Date</strong><span style="color:red">*</span></label>
							<div class="col-lg-4">
						...
						<div class="form-group" >
							...
							<div class="col-lg-4">
								<select name="dc_trade_discount" id="dc_trade_discount" class="form-control" required>
						...
						<div class="form-group">
							...
							<div class="col-lg-4">
								<select name="dc_status" id="dc_status" class="form-control">
									<option value="1" <?php if($dc_status == '1'){?> selected <?php } ?>>Active</option> 
									<option value="0" <?php if($dc_status == '0'){?> selected <?php } ?>>Inactive</option> 
								</select>
							</div>
						</div>
...
<script src="admin-js/edit-discount-coupon-validator.js" type="text/javascript"></script>
<script>
