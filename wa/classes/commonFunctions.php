...
class commonFunctions
{
 ...
 //    }
	...
	public function sendContactUsEmailToCustomer($tdata)
	{
		...
		// SENDSMS TO CUSTOMER - SIGNUP - START
		if($tdata['contactus_contact_no'] != '')
		{
			$tdata_sms = array();
			$tdata_sms['mobile_no'] = $tdata['contactus_contact_no'];
			$tdata_sms['sms_message'] = 'Dear '.$fname.', Thank you for your enquiry. We will get back to you soon.';
			//$this->sendSMS($tdata_sms);	
		}
		// SENDSMS TO CUSTOMER - SIGNUP - END
		
		
		// SENDSMS TO ADMIN - CONTACT US - START
		$tdata_sms_admin = array();
		$tdata_sms_admin['mobile_no'] = '8828033111';
		$tdata_sms_admin['sms_message'] = "Dear Admin, new contact us enquiry recieved with name : ".$fname;
		//$this->sendSMS($tdata_sms_admin);
		// SENDSMS TO ADMIN - CONTACT US - END
		...
		*/	
							...
							<table class="table table-bordered table-striped" style="width: 800px; max-width:100%;">
								<thead>
									<tr>
										...
										<th width="8%">Prod Amt</th>
										<th width="8%">CGST Amt(%)</th>
										<th width="8%">SGST Amt(%)</th>
										<th width="8%">CESS Amt(%)</th>
		...
		if(count($arr_order_cart_details) > 0)
		{
			...
			foreach($arr_order_cart_details as $key => $list_records)
			{
				...
				$prod_final_amt = $list_records['prod_subtotal'] + $prod_cgst_tax_amt + $prod_sgst_tax_amt + $prod_cess_tax_amt;

				// '('.$list_records['prod_ordering_size'].')
				    // <img border="0" height="50" src="'.SITE_URL.'/uploads/'.$list_records['prod_image'].'" alt="'.$list_records['prod_name'].'" >
				$emailHTML .= '     <tr>
										<td>'.$i .'</td>
										<td align="left">'.$list_records['prod_name'].'</td>
										<td align="left">'.$order_cart_delivery_date.'</td>
										<td align="left">'.$list_records['prod_qty'].'</td>
										...
										<td align="left">Rs '.$prod_final_amt.' '.$str_cancel_btn.'</td>
									</tr>';
				$i++;
			}
		}
		...
		{ 
			$emailHTML .= '			<tr><td colspan="10">No Records Found</td></tr>   '; 
		} 
		$emailHTML .= '  		</tbody>
								<tfoot>
									<tr>
										<td colspan="7" align="right" class="text-right"><strong>Sub Total :</strong></td>
										<td colspan="3" align="right" >'.'Rs. '.number_format(floatval($arr_order_details['order_subtotal']), 2, ".", ",").'</td>
									</tr>';
							
		
			$emailHTML .= ' 	<td colspan="7" align="right" class="text-right"><strong>Shipping Charges :</strong></td>
			...
			//{
				$emailHTML .= '	<tr> 
								<td colspan="7" align="right" class="text-right"><strong>Trade Discount:</strong></td>
			...
			$emailHTML .= '	<tr> 
								<td colspan="7" align="right" class="text-right"><strong>Promo Discount'.$coupon_str.':</strong></td>
			...
			{
				$emailHTML .= '	<tr> 
								<td colspan="7" align="right" class="text-right"><strong>CGST:</strong></td>
			...
			{
				$emailHTML .= '	<tr> 
								<td colspan="7" align="right" class="text-right"><strong>SGST:</strong></td>
			...
			{
				$emailHTML .= '	<tr> 
								<td colspan="7" align="right" class="text-right"><strong>CESS:</strong></td>
			...
			$emailHTML .= '	<tr> 
								<td colspan="7" align="right" class="text-right"><strong>Total Tax:</strong></td>
								<td colspan="3" align="right" >'.'Rs. '.number_format(floatval($arr_order_details['order_tax']), 2, ".", ",").'</td>
							</tr>
							<tr>';				
							
			$emailHTML .= ' 	<td colspan="7" align="right" class="text-right"><strong>Total :</strong></td>
		...
		$emailHTML .= '<hr width="800px"/>
				...
				<div style="height: 20px; margin: 5px;width:800px;">
					<div style="float: right; font-size: 15px;text-align: -webkit-auto;">
						ordered via <b style="font-size: 1.3em;"> www.wellnessway4u.com </b>
					</div> 
				</div>
			</div>';
	...
	public function getOrderInvoiceHtmlVendor($invoice,$vendor_id,$show_cancel_btn_status='',$is_admin='')
	{
		$arr_order_details = $this->getOrderDetailsByInvoiceAndVendorId($invoice,$vendor_id);
		$arr_order_cart_details = $this->getOrderCartDetailsByInvoiceAndVendorId($invoice,$vendor_id);
		...
		*/
		...
		$emailHTML .= '	<div  style="margin:0px auto; width: 800px; text-align: center;">
							...
							<div style="height: 50px; margin: 5px;width:100%;" >
								<div style="float: left;font-weight: 700; font-size: 15px;  width: 50%;text-align: -webkit-auto;">
									INVOICE NUMBER : '.$invoice.'-'.$vendor_id.'
								</div> 
		...
		*/	
			...
			foreach($arr_order_cart_details as $key => $list_records)
			{
				...
				if($show_cancel_btn_status == '1')
				{
					...
					else
					{
						if($is_admin == '1')
						{
							$str_cancel_btn = '<br><br><a href="'.BA_URL.'/admin/cancel_order.php?invoice='.$list_records['invoice'].'&ocid='.$list_records['order_cart_id'].'" class="btn btn-danger">Cancel Item</a>';	
						}
						else
						{
							if($this->chkIfItemCaneBeCancelled($list_records['invoice'],$list_records['prod_id'],$list_records['order_cart_delivery_date']))
				...
				$subtotal = $subtotal + $prod_final_amt;	
		...
		$emailHTML .= '  		</tbody>
								<tfoot>
									<tr>
										<td colspan="7" align="right" class="text-right"><strong>Sub Total :</strong></td>
										<td colspan="3" align="right" >'.'Rs. '.number_format(floatval($subtotal), 2, ".", ",").'</td>
									</tr>';
							
			$shipping_amt = 0;
			$emailHTML .= ' 	<td colspan="7" align="right" class="text-right"><strong>Shipping Charges :</strong></td>
								<td colspan="3" align="right" >'.'Rs. '.number_format(floatval($shipping_amt), 2, ".", ",").'</td>
							</tr>';
			...
			if($arr_order_details['order_discount_coupon'] != '')
			{
				if($arr_order_details['order_discount_vendor_id'] == $vendor_id)
				{
					$coupon_str = ' (Coupon Redeemed: '.$arr_order_details['order_discount_coupon'].') ';
					$order_discount = $arr_order_details['order_discount'];
				}	
			}
			...
			$emailHTML .= '	<tr> 
								<td colspan="7" align="right" class="text-right"><strong>Promo Discount'.$coupon_str.':</strong></td>
								<td colspan="3" align="right" >'.'Rs. '.number_format(floatval($order_discount), 2, ".", ",").'</td>
							</tr>
							<tr>';
			...
			$order_total_amt = $subtotal + $shipping_amt - $order_discount + $arr_order_details['order_tax'];				
							
			$emailHTML .= ' 	<td colspan="7" align="right" class="text-right"><strong>Total :</strong></td>
								<td colspan="3" align="right" >'.'Rs. '.number_format(floatval($order_total_amt), 2, ".", ",").'</td>
							</tr>
						</tfoot>
					</table>
				</div>';
						...
						while($r2 = $STH2->fetch(PDO::FETCH_ASSOC))
						{
							...
							else
							{
								...
								if( $STH3->rowCount() > 0 )
								{
									...
									while($r3 = $STH3->fetch(PDO::FETCH_ASSOC))
									{
										$vendor_id = $obj_temp->getVendorIdOfCusine($r3['cusine_id']);		
										$arr_shipping_inclusive_details = $obj_temp->getShippingPriceSPTypewise($city_id,$r2['cart_area_id'],3,$vendor_id);
										...
										else
										{
											...
											if(count($arr_cusine_weight) > 0 )
											{
												...
												for($wq=0;$wq<count($arr_cusine_weight);$wq++)
												{
													...
													if($obj_temp->chkIfCusineHasQuantityWiseShippingPrice($city_id,$r3['cart_area_id'],$arr_cusine_weight[$wq]['cw_qt_cat_id'],$arr_cusine_weight[$wq]['cw_qu_cat_id'],$vendor_id))
													{
												...
												else
												{
													if($r3['cart_area_id'] > 0)
													{
														...
														for($wq=0;$wq<count($arr_cusine_weight);$wq++)
														{
															...
															if($obj_temp->chkIfCusineHasQuantityWiseShippingPrice($city_id,0,$arr_cusine_weight[$wq]['cw_qt_cat_id'],$arr_cusine_weight[$wq]['cw_qu_cat_id'],$vendor_id))
															{
																$temp_wq_qty_val =  $arr_cusine_weight[$wq]['cw_quantity'] * $r3['qty'];
											...
											if($temp_cwsp_count == 0)
											{
												...
												if($obj_temp->chkIfCusineHasAmountWiseShippingPrice($city_id,$r3['cart_area_id'],$vendor_id))
												{
												...
												else
												{
													if($r3['cart_area_id'] > 0)
													{
														...
														if($obj_temp->chkIfCusineHasAmountWiseShippingPrice($city_id,0,$vendor_id))
														{
															$temp_amt_subtotal =  $r3['subtotal'];
											...
											if($temp_cwsp_count == 0 && $temp_amt_count == 0)
											{
												...
												if($obj_temp->chkIfCusineHasPercentWiseShippingPrice($city_id,$r3['cart_area_id'],$vendor_id))
												{
												...
												else
												{
													if($r3['cart_area_id'] > 0)
													{
														...
														if($obj_temp->chkIfCusineHasPercentWiseShippingPrice($city_id,0,$vendor_id))
														{
															$temp_amt_subtotal =  $r3['subtotal'];
						...
						foreach($arr_wt_qty as $tt_wt_qty_k => $tt_wt_qty_v)
						{
							...
							if($tt_wt_qty_v > 0 )
							{
								...
								if(is_array($temp_arr_chk_cwsp) && count($temp_arr_chk_cwsp) > 0)
								{
									...
									$arr_cwsp_exact_match_values = $obj_temp->getCusineShippingPriceQuantityWise($temp_city_id_chk_cwsp,$temp_area_id_chk_cwsp,$temp_applied_on_chk_cwsp,$temp_qty_unit_chk_cwsp,$temp_qty_vendor_id);
						...
						foreach($arr_amt_subtotal as $tt_amt_qty_k => $tt_amt_qty_v)
						{
							...
							if($tt_amt_qty_v > 0 )
							{
								...
								if(is_array($temp_arr_chk_amt) && count($temp_arr_chk_amt) > 0)
								{
									...
									$arr_amt_exact_match_values = $obj_temp->getCusineShippingPriceAmountWise($temp_city_id_chk_amt,$temp_area_id_chk_amt,$temp_amt_vendor_id);
						...
						foreach($arr_per_subtotal as $tt_per_qty_k => $tt_per_qty_v)
						{
							...
							if($tt_per_qty_v > 0 )
							{
								...
								if(is_array($temp_arr_chk_per) && count($temp_arr_chk_per) > 0)
								{
									...
									$arr_per_exact_match_values = $obj_temp->getCusineShippingPricePercentWise($temp_city_id_chk_per,$temp_area_id_chk_per,$temp_per_vendor_id);
	...
	public function doContactUs($tdata)
    {
		...
		try {
			$sql = "INSERT INTO `tblcontactus` (`contactus_city_id`,`contactus_area_id`,`contactus_name`,`contactus_email`,`contactus_contact_no`,
					...
					`contactus_speciality_other`,`contactus_item_id`,`contactus_item_other`,`contactus_no_of_people`,`contactus_comments`,`user_id`,`cu_status`,`cu_add_date`,`requst_type`) 
...
public function getVAIDFromVACATID($va_cat_id,$va_sub_cat_id)
	...
	// written by pramod
	//
	public function getAllDelaveryListOfItem($item_id)
		{
		$DBH = new DatabaseHandler();
		$output = array();
		$date_count = 5;
		...
		$cnt = 0;
		try {
			$sql ="SELECT * FROM `tblcusines` WHERE 1 AND cusine_deleted = '0' AND cusine_status = '1' AND item_id='".$item_id."' ";			
			...
			if( $STH->rowCount() > 0 )
				...
				$r= $STH->fetch(PDO::FETCH_ASSOC);
				//
				$order_cutoff_time=$r['order_cutoff_time'];	
				//
				$cusine_id=$r['cusine_id'];	
				//
			    $temp_date = date('Y-m-d');
					//
					if($r['delivery_date_type'] == 'days_of_month')
			...
			$output['delavery_date'] = array_slice($output, 0, $date_count);
             //			
			$output['order_cutoff_time']=$order_cutoff_time;
			//
			$output['cusine_id']=$cusine_id;
			//
			//echo '<br><pre>';
			//print_r($output);
			//echo '<br></pre>';
 ...
 function countItemDelaviryDate($item_id)
	{
	 $DBH = new DatabaseHandler();
		$output = array();
		$date_count = 5;
		...
		try {
		    ...
		    ///print_r(array_slice($output, 0, $date_count));
			
			$output= count(array_slice($output, 0, $date_count));
	...
	function sendRequestCusine($tdata)
		{
	    $my_DBH = new DatabaseHandler();
        $DBH = $my_DBH->raw_handle();
        $DBH->beginTransaction();
		...
		try {
			$sql = "INSERT INTO `tblcontactus` (`contactus_city_id`,`contactus_area_id`,`contactus_name`,`contactus_email`,`contactus_contact_no`,
					`contactus_parent_cat_id`,`contactus_cat_id`,`contactus_speciality_id`,
					`contactus_item_id`,`contactus_comments`,`user_id`,`cu_status`,`cu_add_date`,`request_type`) 
					VALUES (:contactus_city_id,:contactus_area_id,:contactus_name,:contactus_email,:contactus_contact_no,
					:contactus_parent_cat_id,:contactus_cat_id,:contactus_speciality_id,
					:contactus_item_id,:contactus_comments,:user_id,:cu_status,:cu_add_date,:request_type)";
			$STH = $DBH->prepare($sql);
            $STH->execute(array(
				':contactus_city_id' =>$tdata['request_city_id'],
				':contactus_area_id' =>$tdata['request_area_id'],
				':contactus_name' =>$tdata['request_name'],
				':contactus_email' =>$tdata['request_email'],
				':contactus_contact_no' =>$tdata['request_contact'],
				':contactus_parent_cat_id' =>$tdata['request_parent_id'],
				':contactus_cat_id' =>$tdata['request_cat_id'],
				':contactus_speciality_id' =>$tdata['request_speciality_id'],
				':contactus_item_id' =>$tdata['request_item_id'],
				':contactus_comments' =>$tdata['request_comment'],
				':user_id' => $tdata['user_id'],
				':cu_status' =>$tdata['cu_status'],
				':cu_add_date' =>date('Y-m-d H:i:s'),
				':request_type' =>$tdata['request_type'],
            ));
			$return = $DBH->lastInsertId();
			$DBH->commit();
		} catch (Exception $e) {
			$stringData = '[sendRequestCusine] Catch Error:'.$e->getMessage();
			$this->debuglog($stringData);
            return 0;
        }
        return $return;		
            ...
            public function getDeualtValueOfVendorAccessFormFieldFromVAFID($vaf_id,$field_name)
	{
        ...
        public function getMainProfileOptionLOC($cat_id,$type='1',$multiple='',$default_cat_ids='')
	{
		$DBH = new DatabaseHandler();
		$output = '';
  ...
  public function getMainProfileOptionLOCSub($cat_id,$type='1',$multiple='',$default_cat_ids='')
	{
		$DBH = new DatabaseHandler();
		$output = '';
