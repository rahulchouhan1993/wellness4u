	...
	public function getAllVendors($txtsearch='',$status='',$vendor_parent_cat_id='',$vendor_cat_id='',$country_id='',$state_id='',$city_id='',$area_id='',$added_by_admin='',$added_date_type='',$added_days_of_month='',$added_days_of_week='',$added_single_date='',$added_start_date='',$added_end_date='',$item_id='')
	{
		...
		if($item_id != '')
		{
		 ...
		 $sql = "SELECT TVR.*, TBC1.prct_cat_id AS main_profile_id, TBC1.prct_cat AS main_profile, TBC2.fav_cat_id AS category_id, TBC2.fav_cat AS category_name,
				...
				(SELECT state FROM `tblstates` WHERE `state_id` = TVL.state_id ) AS state_name,
				(SELECT city FROM `tblcities` WHERE `city_id` = TVL.city_id ) AS city_name,
...
public function addVendor($tdata)
    {
		...
		try {
			$sql = "INSERT INTO `tblvendors` (`vendor_username`,`vendor_password`,`vendor_email`,`vendor_name`,`vendor_parent_cat_id`,`vendor_cat_id`,`vendor_status`,`vendor_add_date`,`added_by_admin`,`add_type`,`vendor_mobile`) 
					VALUES (:vendor_username,:vendor_password,:vendor_email,:vendor_name,:vendor_parent_cat_id,:vendor_cat_id,:vendor_status,:vendor_add_date,:added_by_admin,:add_type,:vendor_mobile)";
			$STH = $DBH->prepare($sql);
            $STH->execute(array(
...
public function addEventLocation($tdata)
    ...
    public function addEventCerification($tdata)
    {
		...
		try {
			$sql = "INSERT INTO `tbl_event_certificates`(`user_id`, `user_type`, `event_master_id`, `event_id`, `certificate_type`, `certificate_name`, `certificate_number`, `certificate_issue_date`, `certificate_issue_by`, `certificate_validity_date`, `certificate_scan_copy`) "
                                . "VALUES (:user_id,:user_type,:event_master_id,:event_id,:certificate_type,:certificate_name,:certificate_number,:certificate_issue_date,:certificate_issue_by,:certificate_validity_date,:certificate_scan_copy)";
			$STH = $DBH->prepare($sql);
                        $STH->execute(array(
				':user_id' => $tdata['user_id'],
    ...
    public function getAllEvent($txtsearch='',$status='',$city_id='',$area_id='')
	{
		$DBH = new DatabaseHandler();
		$arr_records = array();
		
		$sql = "SELECT * from tbl_event_master where `is_deleted` = 0  order by event_master_id DESC";	
		//$this->debuglog('[getAllVendors] sql:'.$sql);		
		$STH = $DBH->query($sql);
		if( $STH->rowCount() > 0 )
                {
...
public function getAllEventLocation($txtsearch='',$status='',$city_id='',$area_id='',$event_master_id)
   ...
   public function GetAllEventDetails($txtsearch,$status,$city_id,$area_id)
   {
        $DBH = new DatabaseHandler();
		$arr_records = array();
		$date = date("Y-m-d");
		//$sql = "SELECT * from tbl_event_details where `event_status` = 1  and  `end_date` >= '".$date."'  order by event_id ASC";
               $sql = "SELECT TEM.posted_by,TED.event_add_date,TED.event_id,TED.event_master_id,TED.venue_details,TED.city_id,TED.start_date,TED.event_deleted,TED.start_time,TED.end_date,TED.end_time,TED.event_status,TEM.event_master_id,TEM.reference_number,TEM.organiser_id,TEM.institution_id,TEM.event_name from tbl_event_details TED "
                        . " LEFT JOIN tbl_event_master TEM ON TED.event_master_id = TEM.event_master_id WHERE TED.event_status = 1 AND TED.event_deleted = 0  order by TED.event_id ASC";		
		//$this->debuglog('[getAllVendors] sql:'.$sql);		
		$STH = $DBH->query($sql);
		if( $STH->rowCount() > 0 )
                {
   ...
   public function GETEVENTDETAILSBYID($event_id)
   {
		...
		//$sql = "SELECT * from tbl_event_details where `event_status` = 1  and  `end_date` >= '".$date."'  order by event_id ASC";
               $sql = "SELECT TEM.posted_by,TED.event_add_date,TED.event_id,TED.event_master_id,TED.venue_details,TED.city_id,TED.start_date,TED.start_time,TED.end_date,TED.end_time,TED.event_status,TEM.event_master_id,TEM.reference_number,TEM.organiser_id,TEM.institution_id,TEM.event_name from tbl_event_details TED "
...
public function addEventPrice($tdata)
    ...
    public function getEventPriceAllLocation($event_price_id)
    {
        $DBH = new DatabaseHandler();
        $arr_record = array();
        
        $sql = "SELECT * FROM `tbl_event_loc_price` WHERE `event_price_id` = '".$event_price_id."' AND `culoc_deleted` = '0' ORDER BY default_price DESC, culoc_add_date ASC ";
        $STH = $DBH->query($sql);
        if($STH->rowCount() > 0)
		{
            while($r = $STH->fetch(PDO::FETCH_ASSOC))
    ...
    public function deleteEventPriceLocationByEventPriceId($tdata)
    {
		...
		try {
			$sql = "UPDATE `tbl_event_loc_price` SET 
					...
					`culoc_modified_date` = :culoc_modified_date,
					`deleted_by_admin` = :deleted_by_admin  
					WHERE `event_price_id` = :event_price_id AND `culoc_deleted` != '1' ";
			$STH = $DBH->prepare($sql);
            $STH->execute(array(
				':culoc_deleted' => '1',
				':culoc_modified_date' => date('Y-m-d H:i:s'),
...
public function getEventAllLocationsAndCertifications($event_master_id,$user_id)
    {
        $DBH = new DatabaseHandler();
        $arr_record = array();
        ...
        if($STH->rowCount() > 0)
		{
            while($r = $STH->fetch(PDO::FETCH_ASSOC))
			{
				$r['certificate'] = array();
				//$sql2 = "SELECT * FROM `tblvendorcertifications` WHERE `vendor_id` = '".$vendor_id."' AND `vloc_id` = '".$r['vloc_id']."' AND `vc_cert_deleted` = '0' ORDER BY vc_cert_add_date ASC ";
				$sql2 = "SELECT * from tbl_event_certificates where `event_id` = '".$r['event_id']."' and `user_id` = '".$user_id."'and `user_type` = 'ORG' order by certificate_id ASC";	
                                $STH2 = $DBH->query($sql2);
				if($STH2->rowCount() > 0)
				{
					while($r2 = $STH2->fetch(PDO::FETCH_ASSOC))
    ...
    public function deleteEventLocationByEventMasterId($tdata)
    {
		...
		try {
			$sql = "UPDATE `tbl_event_details` SET 
					...
					WHERE `event_master_id` = :event_master_id AND `event_deleted` != '1' ";
			$STH = $DBH->prepare($sql);
            $STH->execute(array(
				':event_deleted' => '1',
        ...
        $return = false;
            ...
            try {
                    $sql = "UPDATE `tbl_event_certificates` SET 
                                    ...
                                    WHERE `event_master_id` = :event_master_id AND `certificate_deleted` != '1' AND `user_type` ='ORG' ";
                    $STH = $DBH->prepare($sql);
            $STH->execute(array(
...
//            echo '</pre>';
    ...
    public function deleteEventTimeTable($tdata)
    {
            $DBH = new DatabaseHandler();
            $return = false;
            $sql = "DELETE FROM `tbl_event_time_table` WHERE `event_master_id` = '".$tdata['event_master_id']."' ";
            //$this->execute_query($sql);
            $STH = $DBH->query($sql);
            if($STH->rowCount()  > 0)
            {
