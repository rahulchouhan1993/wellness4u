		...
		if($_FILES['aal_import_file']['tmp_name']) {
			...
			if(in_array($_FILES['aal_import_file']['type'],$mimes)){
				...
				//$link = str_replace("YOURUSERID", $sasid, $data[4]);
					...
					if(array_key_exists(3, $data)) $title = sanitize_text_field($data[3]);
					if(array_key_exists(2, $data)) $stats = sanitize_text_field($data[2]);
					if(array_key_exists(4, $data)) $samelink = sanitize_text_field($data[4]);
					if(array_key_exists(5, $data)) $disclosureoff = sanitize_text_field($data[5]);
