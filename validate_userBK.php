...
if(isset($_GET['sess']) && $_GET['sess'] != '')
{
	...
	{
		if(doValiadteUser($email))
		{
			$user_id = getUserId($email);
			$name = getUserFullNameById($user_id);
			$unique_id = getUserUniqueId($user_id);
			
			$tmpref = base64_encode('add_practitioner.php');
			
			$err_msg = 'Welcome '.$name.'! You\'re now registered member  <a href="'.SITE_URL.'/login.php">Click here to Sign In</a> Or <a href="'.SITE_URL.'/login.php?ref='.$tmpref.'">Click here</a> to add Practitioner for Private Counsulting.';		
