...
class Paging
{
	...
	function get_first_page_nav($user_link="",$link_para="",$returnHtml=false)
	{
		...
		if(trim($user_link)=="") $user_link="<span style='padding:10px;'>[First]</span>";
	...
	{
		if($this->total_page<=1) return;
		if(trim($user_link)=="") $user_link="<span style='padding:10px;'>[Last]</span>";
	...
	{
		if($this->total_page<=1) return;
		if(trim($user_link)=="") $user_link="<span style='padding:10px;'>Next</span>";
	...
	{
		if($this->total_page<=1) return;
		if(trim($user_link)=="") $user_link="<span style='padding:10px;'>Prev</span>";
	...
	function get_scroll_prev_page_nav($user_link="",$link_para="",$returnHtml=false)
	{
		...
		if(trim($user_link)=="") $user_link="<span style='padding:10px;'>Prev[$this->scroll_page]</span>";
	...
	{
		if($this->scroll_page>=$this->total_page)return;
		if(trim($user_link)=="") $user_link="<span style='padding:10px;'>Next[$this->scroll_page]</span>";
	...
	function get_number_page_nav($link_para="",$returnHtml=false)
	{
		...
		{
			if($i==$this->page)
				$html .= '&nbsp;<a href="#" style="background-color:#eeeeee;width:20px;color:#000000;">'.($i+1).'</a>&nbsp;';
			else
			{
				if(empty($this->jsHandler))
					$html .= '<a style="background-color:#eeeeee;" href="'.$this->page_name.'?page='.$i.$this->qry_str.'" '.$link_para.'>&nbsp;'.($i+1).'&nbsp;</a> ';
				else 
					$html .= '<a style="background-color:#eeeeee;" href="javascript:void(0);" onclick="'.$this->jsHandler.'(\''.$this->page_name.'\',\'page='.$i.$this->qry_str.'\')" '.$link_para.'>&nbsp;'.($i+1).'&nbsp;</a> ';
			}
		}
