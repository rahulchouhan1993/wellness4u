...
//		if($txtsearch != '')
//		{
//			$sql_search_str = " AND `state` LIKE '%".$txtsearch."%' ";
//		}
//		
//		$sql_status_str = "";
//		if($status != '')
...
//		}
//		
//		$sql = "SELECT * FROM `tblstates` WHERE `state_deleted` = '0'  ".$sql_search_str." ".$sql_status_str." ".$sql_country_str." ".$sql_added_by_admin_str." ".$sql_add_date_str."  ORDER BY `state` ASC";	
//		$STH = $DBH->query($sql);
//		if( $STH->rowCount() > 0 )
//        {
//			while($r= $STH->fetch(PDO::FETCH_ASSOC))
...
//        $return = false;
//
//        $sql = "SELECT * FROM `tblstates` WHERE `state` = '".$state_name."' AND `country_id` = '".$country_id."' AND `state_deleted` = '0' ";
//        $STH = $DBH->query($sql);
//		if( $STH->rowCount() > 0 )
//        {
//            $return = true;
...
//            $STH->execute(array(
//				':country_id' => $tdata['country_id'],
//				':state' => addslashes($tdata['state_name']),
//				':state_status' => '1',
//				':added_by_admin' => $tdata['admin_id'],
//				
//            ));
...
//        $return = false;
//
//        $sql = "SELECT * FROM `tblstates` WHERE `state` = '".$state_name."' AND `state_id` != '".$state_id."' AND `country_code` = '".$country_id."' `state_deleted` = '0' ";
//        $STH = $DBH->query($sql);
//		if( $STH->rowCount() > 0 )
//        {
//            $return = true;
...
//	}
//	
//         public function getFavCategoryRamakant11($fav_cat_type_id,$fav_cat_id)
//	{
//            $this->connectDB();
//            $option_str = '';		
...
//	
//        
//    public function getProfSpecialityOfferedOptionVivek($fav_cat_type_id)
//            
//            {
//		$DBH = new DatabaseHandler();
//		$arr_records = array();
...
//        vivek start 
       ...
       public function addVendor($tdata) 
        ...
        $DBH->beginTransaction();
         $vendor_add_date=date('Y-m-d H:i:s');
        $sql="insert into `tbl_profusers` (`vendor_status`,`vendor_add_date`,`added_by_admin`,`va_id`,`new_vendor`,`username`,`password`,`email`,`vendor_name`,`service_cat`,`vendor_cat_id`) value('".$tdata['vendor_status']."','".$vendor_add_date."','".$tdata['admin_id']."','".$tdata['va_id']."','".$tdata['new_vendor']."','".$tdata['vendor_username']."','".md5($tdata['vendor_password'])."','".$tdata['vendor_username']."','".$tdata['vendor_name']."','".$tdata['service_cat']."','".$tdata['vendor_cat_id']."')";
        $STH = $DBH->query($sql);
        $vendor_id = $DBH->lastInsertId();
...
//            } 
        ...
        $return = 0;
         ...
         $vc_cert_add_date=date('Y-m-d H:i:s');
        $sql="insert into `tbl_profusers_certifications` (`vendor_id`,`vloc_id`,`vc_cert_type_id`,`vc_cert_name`,`vc_cert_no`,`vc_cert_reg_date`,`vc_cert_validity_date`,`added_by_admin`,`vc_cert_add_date`,`vc_cert_issued_by`,`vc_cert_scan_file`,`vc_cert_status`) value('".$tdata['vendor_id']."','".$tdata['vloc_id']."','".$tdata['vc_cert_type_id']."','".$tdata['vc_cert_name']."','".$tdata['vc_cert_no']."','".$tdata['vc_cert_reg_date']."','".$tdata['vc_cert_validity_date']."','".$tdata['admin_id']."','".$vc_cert_add_date."','".$tdata['vc_cert_issued_by']."','".$tdata['vc_cert_scan_file']."','".$tdata['vc_cert_status']."')";
        $STH = $DBH->query($sql);
        $return = $DBH->lastInsertId();
//        if($STH->rowCount() > 0) 
//            {
...
//            } 
       ...
       public function addVendorLocation($tdata) 
        ...
        $DBH->beginTransaction();
        $vloc_add_date=date('Y-m-d H:i:s');
        $sql="insert into `tbl_profusers_location` (`vloc_default`,`vloc_status`,`added_by_admin`,`vloc_add_date`,`vloc_parent_cat_id`,`vloc_cat_id`,`vloc_speciality_offered`,`vloc_doc_file`,`vloc_menu_file`,`vendor_id`,`contact_person`,`contact_person_title`,`contact_email`,`contact_designation`,`contact_number`,`contact_remark`,`country_id`,`state_id`,`city_id`,`area_id`) value('".$tdata['vloc_default']."','1','".$tdata['admin_id']."','".$vloc_add_date."','".$tdata['vloc_parent_cat_id']."','".$tdata['vloc_cat_id']."','".$tdata['vloc_speciality_offered']."','".$tdata['vloc_doc_file']."','".$tdata['vloc_menu_file']."','".$tdata['vendor_id']."','".$tdata['contact_person']."','".$tdata['contact_person_title']."','".$tdata['contact_email']."','".$tdata['contact_designation']."','".$tdata['contact_number']."','".$tdata['contact_remark']."','".$tdata['country_id']."','".$tdata['state_id']."','".$tdata['city_id']."','".$tdata['area_id']."')";
        $STH = $DBH->query($sql);
        $return = $DBH->lastInsertId();
//        if($STH->rowCount() > 0) 
//            {
