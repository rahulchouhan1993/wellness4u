<?php
function getCommaSepratedSituatinsForTriggers($user_id,$date,$bms_id,$bms_type)
{
    global $link;
    $output = '';

    $sql = "SELECT * FROM `tblusersmdt` "
            . "WHERE `user_id` = '".$user_id."' AND `mdt_date` = '".$date."' AND `bms_id` = '".$bms_id."' "
            . "AND `bms_entry_type` = 'trigger' AND `bms_type` = '".$bms_type."' AND `mdt_old_data` = '0' ORDER BY mdt_add_date DESC ";
    //echo '<br>'.$sql.'<br>';
    ...
    if( ($result) && (mysql_num_rows($result) > 0) )
    {
        ...
        {
            $time = stripslashes($row['mdt_time']);
            $duration = stripslashes($row['mdt_duration']);

            $sql2 = "SELECT * FROM `tblusersmdt` "
                . "WHERE `user_id` = '".$user_id."' AND `mdt_date` = '".$date."' AND `mdt_time` = '".$time."' AND `mdt_duration` = '".$duration."' "
                . "AND `bms_entry_type` = 'situation' AND `mdt_old_data` = '0' ORDER BY mdt_add_date DESC ";
            //echo '<br>'.$sql2.'<br>';
            ...
            if( ($result2) && (mysql_num_rows($result2) > 0) )
            {
                ...
                {
                    $situation = getBobyMainSymptomName($row2['bms_id']);
                    $scale = stripslashes($row2['scale']);
                    if($scale == '')
                    {
                        $scale = '0';
                    }

                    $output .= $situation.' ('.$scale.') ,';
                }    
            } 
        }
...
function getUserAcceptedReportsSetOptions($user_id,$pro_user_id,$report_id,$user_set_id)
{
    global $link;
    $option_str = '';
    list($str_report_id,$str_permission_type) = getUserAcceptedReportId($user_id,$pro_user_id);
    ...
    {	
        ...
        for($i=0;$i<count($arr_report_id);$i++)
        { 
            if( ($arr_report_id[$i] == $report_id) && ($report_id != '') )
            {
                

                if($arr_permission_type[$i] == '2')
                {
                    if($arr_permission_type[$i] == $user_set_id)
                    {
                        $sel = ' selected ';
                    }
                    else
                    ...
                    $option_str = '<option value="2" '.$sel.'>Standard Set</option>';
                }
                elseif($arr_permission_type[$i] == '3')
                {
                    if($arr_permission_type[$i] == $user_set_id)
                    ...
                    $option_str = '<option value="3" '.$sel.'>Adviser Set</option>';
                }	
                elseif($arr_permission_type[$i] == '1')
                {
                    if($user_set_id == '1')
                    {
                        $sel1 = ' selected ';
                    ...
                    $option_str = '<option value="1" '.$sel1.'>All</option>';
                    $option_str .= '<option value="2" '.$sel2.'>Standard Set</option>';
                    $option_str .= '<option value="3" '.$sel3.'>Adviser Set</option>';
                }
            }
        }
    }
...
function getUserAcceptedReportsOptionsNew($user_id,$pro_user_id,$report_id)
{
    global $link;
    $option_str = '';
    list($str_report_id,$str_permission_type) = getUserAcceptedReportId($user_id,$pro_user_id);
        ...
        { 
            ...
            if( ($result) && (mysql_num_rows($result) > 0) )
            {
                while($row = mysql_fetch_array($result) ) 
                {
                    ...
                    $option_str .= '<option value="'.$row['report_id'].'" '.$sel.'>'.stripslashes($row['report_name']).'</option>';
                }
            }
        }
    }
...
function getAdvisersUserOptions($user_id,$pro_user_id)
{
	...
	$email = getProUserEmailById($pro_user_id);	
...
function getAdvisersUserOptionsMulti($arr_user_id,$practitioner_id)
{
	...
	else
	{
		//$sql = "SELECT * FROM `tbladviserreferrals` AS tar LEFT JOIN `tblusers` AS tpu ON tar.user_email = tpu.email  WHERE tar.pro_user_id = '".$practitioner_id."' AND tar.request_status = '1' ORDER BY tpu.name ASC";
                ...
                $email = getProUserEmailById($practitioner_id);	
		
                $sql = "SELECT * FROM `tbladviserreferrals` AS tar "
                        . "LEFT JOIN `tblusers` AS tpu ON tar.pro_user_id = tpu.user_id  "
                        . "WHERE tar.user_email = '".$email."' AND tar.invite_by_user = '1' "
                        . "ORDER BY tpu.name ASC";
                //echo $sql;
                $result = mysql_query($sql,$link);
                if( ($result) && (mysql_num_rows($result) > 0) )
                {
                        while($row = mysql_fetch_array($result) ) 
                        {
                                ...
                                if(in_array($row['pro_user_id'],$arr_user_id))
                                {
                                        $sel = ' selected ';
                                }
                                else
                ...
                $sql = "SELECT * FROM `tbladviserreferrals` AS tar "
                        . "LEFT JOIN `tblusers` AS tpu ON tar.user_email = tpu.email  "
                        . "WHERE tar.pro_user_id = '".$practitioner_id."' AND tar.invite_by_user = '0' "
                        . "ORDER BY tpu.name ASC";
                //echo $sql;
                $result = mysql_query($sql,$link);
                if( ($result) && (mysql_num_rows($result) > 0) )
...
function getUsersAdviserOptions($user_id,$pro_user_id)
{
	...
	//$sql = "SELECT * FROM `tbladviserreferrals` AS tar LEFT JOIN `tblprofusers` AS tpu ON tar.pro_user_id = tpu.pro_user_id  WHERE tar.user_email = '".$email."' AND tar.request_status = '1' ORDER BY tpu.name ASC";
        $sql = "SELECT tar.* , tpu.name FROM `tbladviserreferrals` AS tar "
                ...
                . "WHERE tar.user_email = '".$email."' AND tar.invite_by_user = '0' AND tar.request_status = '1'  "
                . "ORDER BY tpu.name ASC";
	//echo $sql;
	$result = mysql_query($sql,$link);
	if( ($result) && (mysql_num_rows($result) > 0) )
	{
        ...
        $sql = "SELECT tar.*, tpu.name , tpu.pro_user_id AS puser_id FROM `tbladviserreferrals` AS tar "
                ...
                . "WHERE tar.pro_user_id = '".$user_id."' AND tar.invite_by_user = '1' AND tar.request_status = '1' "
                . "ORDER BY tpu.name ASC";
	//echo $sql;
        $result = mysql_query($sql,$link);
        if( ($result) && (mysql_num_rows($result) > 0) )
...
function getUsersAcceptedAdviserIds($user_id)
{
	global $link;
	$option_str = '';	
	$email = getUserEmailById($user_id);	
...
function getUsersAcceptedAdviserOptions($user_id,$pro_user_id)
{
	global $link;
	$option_str = '';	
	$email = getUserEmailById($user_id);	
...
function getAdvisersAcceptedUserOptions($pro_user_id,$user_id)
{
	global $link;
	$option_str = '';	
	$email = getProUserEmailById($pro_user_id);	
	...
	$sql = "SELECT * FROM `tbladviserreferrals` AS tar "
                . "LEFT JOIN `tblusers` AS tpu ON tar.pro_user_id = tpu.user_id  "
                . "WHERE tar.user_email = '".$email."' AND tar.invite_by_user = '1' AND (tar.request_status = '1' OR tar.request_status = '3') "
                . "ORDER BY tpu.name ASC";
	//echo $sql;
	$result = mysql_query($sql,$link);
	if( ($result) && (mysql_num_rows($result) > 0) )
	{
        ...
        $sql = "SELECT tar.* , tpu.name , tpu.user_id FROM `tbladviserreferrals` AS tar "
                . "LEFT JOIN `tblusers` AS tpu ON tar.user_email = tpu.email  "
                . "WHERE tar.pro_user_id = '".$pro_user_id."' AND tar.invite_by_user = '0' AND (tar.request_status = '1' OR tar.request_status = '3') "
                . "ORDER BY tpu.name ASC";
	//echo $sql;
	$result = mysql_query($sql,$link);
	if( ($result) && (mysql_num_rows($result) > 0) )
...
function getAdvisersAllUserOptions($pro_user_id,$user_id)
{
	global $link;
	$option_str = '';	
	$email = getProUserEmailById($pro_user_id);	
	...
	if( ($result) && (mysql_num_rows($result) > 0) )
	{
        ...
        $sql = "SELECT tar.* , tpu.name , tpu.user_id FROM `tbladviserreferrals` AS tar "
                . "LEFT JOIN `tblusers` AS tpu ON tar.user_email = tpu.email  "
                . "WHERE tar.pro_user_id = '".$pro_user_id."' AND tar.invite_by_user = '0' "
                . "ORDER BY tpu.name ASC";
	//echo $sql;
	$result = mysql_query($sql,$link);
	if( ($result) && (mysql_num_rows($result) > 0) )
	{
		while($row = mysql_fetch_array($result) ) 
		{
			...
			$option_str .= '<option value="'.$row['user_id'].'" '.$sel.'>'.stripslashes($row['user_name']).'</option>';
...
function getAdviserQueryPageOptions($page_id,$user_id,$pro_user_id,$show_all)
{
	...
	//echo '<br>page_id = '.$page_id;
        ...
        //echo '<br>pro_user_id = '.$pro_user_id;
        //echo '<br>show_all = '.$show_all;
	if($page_id == '0_0')
	{
		$sel = ' selected ';
	}
...
function getUserQueryPageOptions($page_id,$user_id,$pro_user_id,$show_all = '1')
{
	global $link;
	$option_str = '';
...
function chkIfUserRequestDateisUpdated($pro_user_id,$user_id)
{
	...
	$user_email = getProUserEmailById($user_id);
	$sql = "SELECT * FROM `tbladviserreferrals` "
                . "WHERE `pro_user_id` = '".$pro_user_id."' AND `user_email` = '".$user_email."' AND `request_accept_date` = '0000-00-00 00:00:00'";
	$result = mysql_query($sql,$link);
	if( ($result) && (mysql_num_rows($result) > 0) )
	{
		$return = false;
...
function doAcceptUserInvitation($ar_id,$user_id,$pro_user_id)
{
	global $link;
	$return = false;
	
	if(chkIfUserRequestDateisUpdated($pro_user_id,$user_id))
	{
		$sql = "UPDATE `tbladviserreferrals` SET "
	...
	{
		$sql = "UPDATE `tbladviserreferrals` SET "
                        . "`request_accept_date` = NOW() , `request_status` = '1' , `last_status_updated_by_adviser` = '1' "
                        . "WHERE `ar_id` = '".$ar_id."'";
	}
...
function declineAdviserInvitation($ar_id,$user_id,$pro_user_id,$status_reason)
{
	global $link;
	$return = false;
	
	$sql = "UPDATE `tbladviserreferrals` SET "
                . "`request_accept_date` = NOW() , `request_status` = '2' , `last_status_updated_by_adviser` = '0' "
                . "WHERE `ar_id` = '".$ar_id."' ";
	$result = mysql_query($sql,$link);
	if($result)
	{
		$sql2 = "INSERT INTO `tbladviseractivation`(`ar_id`,`user_id`,`pro_user_id`,`aa_status`,`aa_status_reason`,`aa_status_updated_by_adviser`) VALUES('".$ar_id."','".$user_id."','".$pro_user_id."','2','".addslashes($status_reason)."','0')";
		$result2 = mysql_query($sql2,$link);
		if($result2)
		{
...
function declineUserInvitation($ar_id,$user_id,$pro_user_id,$status_reason)
{
	global $link;
	$return = false;
...
function deactivateUserInvitation($ar_id,$user_id,$pro_user_id,$status_reason)
{
	global $link;
	$return = false;
	...
	if($result)
	{
		$sql2 = "INSERT INTO `tbladviseractivation`(`ar_id`,`user_id`,`pro_user_id`,`aa_status`,`aa_status_reason`,`aa_status_updated_by_adviser`) VALUES('".$ar_id."','".$user_id."','".$pro_user_id."','3','".addslashes($status_reason)."','1')";
		$result2 = mysql_query($sql2,$link);
		if($result2)
		{
...
function activateUserInvitation($ar_id,$user_id,$pro_user_id,$status_reason)
{
	global $link;
	$return = false;
	...
	if($result)
	{
		$sql2 = "INSERT INTO `tbladviseractivation`(`ar_id`,`user_id`,`pro_user_id`,`aa_status`,`aa_status_reason`,`aa_status_updated_by_adviser`) VALUES('".$ar_id."','".$user_id."','".$pro_user_id."','1','".addslashes($status_reason)."','1')";
		$result2 = mysql_query($sql2,$link);
		if($result2)
		{
...
function showDeclineAdviserInvitationPopup($ar_id,$pro_user_id)
{
	global $link;
	$output = '';
	$output .= '<div style="margin-top:15px;width:520px;height:400px;overflow:scroll;">';
...
function showDeclineUserInvitationPopup($ar_id,$pro_user_id)
{
	global $link;
	$output = '';
	$output .= '<div style="margin-top:15px;width:520px;height:400px;overflow:scroll;">';
...
function showDeactivateUserInvitationPopup($ar_id,$pro_user_id)
{
	global $link;
	$output = '';
	$output .= '<div style="margin-top:15px;width:520px;height:400px;overflow:scroll;">';
	...
	$output .= '		<tr>
		...
		$output .= '		<tr>
							<td colspan="2" height="30" align="left" valign="middle"><strong><input type="button" name="btnDoAccept" id="btnDoAccept" value="Deactivate" onclick="deactivateUserInvitation(\''.$ar_id.'\',\''.$pro_user_id.'\')" />&nbsp;&nbsp;<input type="button" name="btnCancelPopup" id="btnCancelPopup" value="Cancel" /></strong></td>
...
function showActivateUserInvitationPopup($ar_id,$pro_user_id)
{
	global $link;
	$output = '';
	$output .= '<div style="margin-top:15px;width:520px;height:400px;overflow:scroll;">';
	...
	$output .= '		<tr>
		...
		$output .= '		<tr>
							<td colspan="2" height="30" align="left" valign="middle"><strong><input type="button" name="btnDoAccept" id="btnDoAccept" value="Activate" onclick="activateUserInvitation(\''.$ar_id.'\',\''.$pro_user_id.'\')" />&nbsp;&nbsp;<input type="button" name="btnCancelPopup" id="btnCancelPopup" value="Cancel" /></strong></td>
...
function getAllMyAdviserInvitations($user_email,$pro_user_id)
{
    ...
    $sql = "SELECT * FROM `tbladviserreferrals` AS tar "
            . "LEFT JOIN `tblprofusers` AS tpf ON tar.pro_user_id = tpf.pro_user_id "
            . "WHERE tar.user_email = '".$user_email."' AND tar.invite_by_user = '0' AND (tar.request_status = '1' OR tar.request_status = '3') ".$str_sql_pro_user_id." "
    ...
    $puid = getUserId($user_email);  
    
    if($pro_user_id != '')
	{
		$str_sql_pro_user_email = " AND tpf.pro_user_id = '".$pro_user_id."' ";
	}
	else
	{
    ...
    $sql = "SELECT * FROM `tbladviserreferrals` AS tar "
            . "LEFT JOIN `tblprofusers` AS tpf ON tar.user_email = tpf.email "
            . "WHERE tar.pro_user_id = '".$puid."' AND tar.invite_by_user = '1' AND (tar.request_status = '1' OR tar.request_status = '3') ".$str_sql_pro_user_email." "
            . "ORDER BY tar.request_sent_date DESC";
	$result = mysql_query($sql,$link);
	if( ($result) && (mysql_num_rows($result) > 0) )
	{
...
function getAllMyUserInvitations($user_email,$pro_user_id)
{
	global $link;
	$temp_arr = array();
...
function getAllAdviserUserReferrals($pro_user_id,$user_id,$status,$invite_start_date,$invite_end_date,$status_start_date,$status_end_date)
{
    global $link;
	$temp_arr = array();
        ...
        if($user_id != '')
        {
            $uemail = getUserEmailById($user_id);
            $str_sql_user_id1 = " AND `user_email` = '".$uemail."' ";
            $str_sql_user_id2 = " AND `pro_user_id` = '".$user_id."' ";
        }
        else
        {
            $str_sql_user_id1 = "";
    ...
    $sql = "SELECT * FROM `tbladviserreferrals` WHERE "
            . "(`pro_user_id` = '".$pro_user_id."' ".$str_sql_user_id1." ".$str_sql_status1." ".$str_sql_invite_date1." ".$str_sql_status_date1." AND `invite_by_user` = '0') "
...
function getAllUserAdviserReferrals($user_id,$pro_user_id,$status,$invite_start_date,$invite_end_date,$status_start_date,$status_end_date)
{
    global $link;
	$temp_arr = array();
        ...
        if($pro_user_id != '')
        {
            $puemail = getProUserEmailById($pro_user_id);
...
function addUsersReferral($pro_user_id,$user_email,$user_name,$message,$new_user,$invite_by_user)
{
	global $link;
	$ar_id = 0;
...
function chkIfRequestAlreadySentByAdviser($pro_user_id,$user_email)
{
    ...
    $sql = "SELECT * FROM `tbladviserreferrals` WHERE `pro_user_id` = '".$pro_user_id."' AND `user_email` = '".$user_email."' AND `invite_by_user` = '0' ";
	$result = mysql_query($sql,$link);
	if( ($result) && (mysql_num_rows($result) > 0) )
	{
		$row = mysql_fetch_array($result);
...
function chkIfRequestAlreadySentByUser($pro_user_id,$user_email)
{
    global $link;
	$return = false;
