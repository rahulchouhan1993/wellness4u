...
if(isset($_POST['save']))
{
	...
	$res=$obj->update_RSS_buttons($item_id,$data);

	if($res)
	{	
		$_SESSION['banner_msg'] = 'Buttons Data Updated Successfully!';
		header("Location: index.php?mode=edit_rss_feed_item&id=".$item_id."&rfuid=".$id);
		exit(0);
	}

}

$data=$obj->get_button_data_RSS($item_id);



if(empty($data))
{
	$data=$obj->get_default_button_data_by_menuID($menu_id="42");
}
...
<!--notification_contents-->
	...
	<tbody>
		...
		<tr>
			<td>
				<br>
				<a href="index.php?mode=edit_rss_feed_item&id=<?=$item_id?>&rfuid=<?=$id?>"><button class="btn btn-default">Back</button></a>
