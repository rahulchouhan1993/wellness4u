...
if(isset($_POST['btnSubmit']))
{
	...
	elseif($obj->chkSubAdminUserNameExists_edit($username,$admin_id))
	{
		$error = true;
		$err_msg .= 'This user name already registered</div>';
	}
	...
	elseif($obj->chkSubAdminEmailExists_edit($email,$admin_id))
	{
		$error = true;
	...
	if(!$error)
	{
		...
		if($obj->updateSubAdmin($admin_id,$email,$username,$status,$fname,$lname,$address,$dob,$contact_no,$country,$state,$city,$menu_comma_separated,$permissions_comma_separated))
		{
...
elseif(isset($_GET['uid']))
{
	...
	list($return,$username,$email,$fname,$lname,$dob,$contact_no,$address,$city,$state,$country,$admin_menu_id,$admin_action_id,$status) = $obj->getSubAdminDetails($admin_id);
	
	//echo $admin_menu_id;
	
	$arr_chk_menu = explode(",", $admin_menu_id);
	
	$arr_chk_permission = explode(",", $admin_action_id);
	
	//print_r($arr_admin_action_id);
			...
			<td>
				...
				<tbody>
					<tr>
						...
						<td style="background-image: url(images/mainbox_title_bg.gif);" class="mainbox-title" width="100%">Edit Sub Admin </td>
				...
				<tbody>
					<tr>
						<td class="mainbox-body">
							<form action="#" method="post" name="frmedit_admin" id="frmedit_admin" enctype="multipart/form-data" >
							...
							<tbody>
                               ...
                               <tr>
                                ...
                                <td align="center">&nbsp;</td>
                                  ...
                                  <?php list($arr_admin_menu_id,$arr_menu_name,$arr_menu_href) = $obj->getMenu();
                               ...
                               <td align="center">&nbsp;</td>
                                <td align="left"><input type="checkbox" name="menu[]" id="menu_<?php echo $arr_admin_menu_id[$i1]; ?>" value="<?php echo $arr_admin_menu_id[$i1]; ?>" onClick="ShowOrHideCheckBox('<?php echo $arr_admin_menu_id[$i1]; ?>');" <?php if (in_array($arr_admin_menu_id[$i1] , $arr_chk_menu)) { ?> checked="checked" <?php } ?> />&nbsp;<strong><?php echo $arr_menu_name[$i1]; ?></strong></td>
                                 ...
                                 <?php list($arr_admin_action_id,$arr_action_name) = $obj->getActions($arr_admin_menu_id[$i1]);
