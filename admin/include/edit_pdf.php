...
{
	$stress_buster_pdf_id = $_POST['hdnstress_buster_pdf_id'];
	$pdf = $_POST['hdn_pdf'];
	...
	else
	{
		...
		if(!$error)
		{
			if($obj->Update_StressBusterPDF($step,$pdf,$credit,$credit_url,$pdf_title,$status,$day,$stress_buster_pdf_id))
				{
	...
	{
		$stress_buster_pdf_id = $_GET['uid'];

		list($step,$pdf,$credit,$credit_url,$pdf_title,$day,$status) = $obj->getSressBusterPDFDetails($stress_buster_pdf_id);
	...
	{
		header('location: index.php?mode=manage_pdf');
	}	
				...
				<tbody>
					<tr>
						<td class="mainbox-body">
							...
							<input type="hidden" name="hdnstress_buster_pdf_id" value="<?php echo $stress_buster_pdf_id;?>" />
