					...
					while (($data = fgetcsv($handle, 0, $field_separate_char,$field_enclose_char)) !== FALSE) 
					{
						...
						else 
						{
							if(!empty($data) ) 
							{
								...
								if($data[1] != '')
								{
									$height_feet_inch = $data[1];
									$height_inch = $data[2];
									$height_cms = $data[3];
									if(!$obj->chkHeightExists($height_cms))
									{
										if($obj->addHeight($height_feet_inch,$height_inch,$height_cms))
										{
											$succ_cnt++;	
										}
										else
			...
			<td>
				...
				<tbody>
					<tr>
						...
						<td style="background-image: url(images/mainbox_title_bg.gif);" class="mainbox-title" width="100%">Import Height</td>
				...
				<tbody>
					<tr>
						<td class="mainbox-body">
							<form action="#" method="post" name="frmimport_heights" id="frmimport_heights" enctype="multipart/form-data" >
