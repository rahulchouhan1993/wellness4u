...
if(isset($_POST['btnSubmit']))
{
	...
	$cs_name = trim($_POST['hdncs_name']);
	$cs_value = strip_tags(trim($_POST['cs_value']));
	...
	if(!$error)
	{
		if($obj->UpdateCommonMessage($cs_id,$cs_value))
		{
			$err_msg = "Record Updateds Successfully!";
			header('location: index.php?mode=common_messages_adviser&msg='.urlencode($err_msg));
		}
		else
		{
			$error = true;
...
{
	$cs_id = $_GET['id'];
	list($cs_name,$cs_value) = $obj->getCommonMessageDetails($cs_id);
	if($cs_name == '')
	{
		header('location: index.php?mode=common_messages_adviser');
	}
}
else
{
						...
						<td class="mainbox-body">
							...
							<tbody>
								<tr>
									...
									<td width="75%" align="left"><strong><?php echo $cs_name; ?></strong></td>
								</tr>
								<tr>
