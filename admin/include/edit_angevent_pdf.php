...
{
	$pdf_id = $_POST['hdnangervent_pdf_id'];
	$pdf = $_POST['hdn_pdf'];
	...
	else
	{
		...
		if(!$error)
		{
			if($angervent->Update_AngerVentPDF($step,$pdf,$credit,$credit_url,$pdf_title,$status,$day,$pdf_id))
				{
					$msg = "Record Edited Successfully!";
					header('location: index.php?mode=angervent_pdf&msg='.urlencode($msg));
				}
			else
				{
					$error = true;
	...
	{
		$angervent_pdf_id = $_GET['uid'];

		list($step,$pdf,$credit,$credit_url,$pdf_title,$day,$status) = $angervent->getAngerventPDFDetails($angervent_pdf_id);
	...
	{
		header('location: index.php?mode=angervent_pdf');
	}	
				...
				<tbody>
					<tr>
						<td class="mainbox-body">
							<form action="#" method="post" name="frmedit_angervent" id="frmedit_angervent" enctype="multipart/form-data" >
							<input type="hidden" name="hdnangervent_pdf_id" value="<?php echo $angervent_pdf_id;?>" />
