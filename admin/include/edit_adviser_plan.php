...
{
	$ap_id = $_POST['hdnap_id'];
	$ap_status = trim($_POST['ap_status']);
	...
	if(!$error)
	{
		...
		if($obj->updateAdviserPlan($ap_id,$ap_name,$ap_months_duration,$ap_amount,$ap_currency,$ap_default,$ap_status,$arr_apa_id,$arr_apa_value,$arr_apc_id,$arr_apc_value,$ap_start_date,$ap_end_date,$apct_id,$country_id,$str_state_id,$str_city_id,$str_place_id,$ap_criteria_from,$ap_criteria_to,$ap_show))
		{
...
{
	$ap_id = $_GET['id'];
	list($ap_name,$ap_months_duration,$ap_amount,$ap_currency,$ap_status,$ap_default,$arr_apa_id,$arr_apa_name,$arr_apa_value,$arr_apc_id,$arr_apc_value,$ap_start_date,$ap_end_date,$apct_id,$country_id,$state_id,$city_id,$place_id,$ap_criteria_from,$ap_criteria_to,$ap_show) = $obj->getAdviserPlanDetails($ap_id);
	...
	if($ap_start_date == '0000-00-00')
	{
		$ap_start_date = '';
	}
	else
	{
		$ap_start_date = date('d-m-Y',strtotime($ap_start_date));
	}	
	
	if($ap_end_date == '0000-00-00')
	{
		$ap_end_date = '';
	}
	else
	{
		$ap_end_date = date('d-m-Y',strtotime($ap_end_date));
	}	
				...
				<tbody>
					<tr>
						<td class="mainbox-body">
							<form action="#" method="post" name="frmedit_sleep" id="frmedit_sleep" enctype="multipart/form-data" onsubmit=" return confirmEditAdviserPlan();" >
							<input type="hidden" name="hdnap_id" id="hdnap_id" value="<?php echo $ap_id;?>" />
							...
							<tbody>
                            	<tr>
									...
									<td width="65%" align="left">
                                        <select id="ap_status" name="ap_status">
                                            <option value="1" <?php if($ap_status == '1'){ ?> selected <?php } ?>>Active</option>
                                            <option value="0" <?php if($ap_status == '0'){ ?> selected <?php } ?>>Inactive</option>
                                        </select>
                                    </td>
								</tr>
                                <tr>
								...
								<?php /*?><tr>
									...
									<td align="center"><strong>:</strong></td>
									<td align="left"><input type="checkbox" name="ap_default" id="ap_default" value="1" <?php if($ap_default == '1'){?> checked="checked" <?php }?> onclick="confirmAdviserDefaultPlan('<?php echo $ap_id;?>')"  ></td>
								</tr>
								<tr>
									<td colspan="3" align="center">&nbsp;</td>
								</tr><?php */?>
                                ...
                                <tr>
									...
									<td align="left">
                                    	...
                                    	<table align="left" border="0" width="100%" cellpadding="0" cellspacing="0">
                                        	<tr>
                                            	<td width="10%" align="left"><input type="checkbox" name="apa_value_<?php echo $i;?>" id="apa_value_<?php echo $i;?>" value="1" <?php if($arr_apa_value[$i] == '1'){?> checked="checked" <?php }?> onclick="return false"  ></td>
                                                <td width="40%" align="left">
                                                	<select name="apc_id_<?php echo $i;?>" id="apc_id_<?php echo $i;?>" style="width:200px;" disabled="disabled">
                                                ...
                                                <td width="50%" align="left"><input readonly="readonly" type="text" name="apc_value_<?php echo $i;?>" id="apc_value_<?php echo $i;?>" value="<?php echo $arr_apc_value[$i]; ?>" ></td>
                                            </tr>    
                                   		</table> 
                                    </td>
								</tr>
