...
{
	$slider_id = $_POST['hdnslider_id'];  
	$parents_slider_id = $_POST['hdnparents_slider_id'];
	...
	{
		$slider_image = strip_tags(trim($_POST['hdnslider_image']));
	}

	if(!$error)
	{
		if($obj->updateSliderContent($slider_title,$slider_desc,$slider_image,$parents_slider_id,$slider_link,$slider_id))
		{
		 	$msg = "Record Updated Successfully!";
			header('location: index.php?mode=view_sliders&msg='.urlencode($msg).'&uid='.$parents_slider_id.'');
		}
		else
		{
			$error = true;
...
{
	$slider_id = $_GET['id'];
	list($slider_title,$slider_desc,$slider_image,$parents_slider_id,$slider_link) = $obj->getSliderDetails($slider_id);
	
	if($slider_title == '')
	{
		header('location: index.php?mode=view_sliders');	
	}	
}	
else
{
				...
				<tbody>
					<tr>
						<td class="mainbox-body">
							...
							<input type="hidden" name="hdnslider_id" id="hdnslider_id" value="<?php echo $slider_id;?>" />
							<input type="hidden" name="hdnslider_image" id="hdnslider_image" value="<?php echo $slider_image;?>" />
                            <input type="hidden" name="hdnparents_slider_id" id="hdnparents_slider_id" value="<?php echo $parents_slider_id;?>" />
							...
							<tbody>
								...
								<tr>
									...
									if($slider_image != '')
									{ ?>
										<img border="0" width="60" src="<?php echo SITE_URL.'//uploads/'.$slider_image;?>" />
									<?php
									}
									?>
									</td>
