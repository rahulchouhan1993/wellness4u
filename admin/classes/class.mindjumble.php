...

{
...

	{
...

		{
			...
			$sql = "SELECT * FROM `tblmindjumble` ORDER BY status DESC,box_add_date DESC";
...

		{
			...
			$sql = "SELECT * FROM `tblmindjumble` WHERE  box_title like '%".$search."%' 
					ORDER BY status DESC,box_add_date DESC";
...

		{
...

			{
				...
				if($row['box_type']=='rss')
				{
					$banner=$this->get_rss_feed_title($row['box_banner']);
				}
				elseif($row['box_type']=='Video')
				{
					$banner='<a href="'.stripslashes($row['box_banner']).'" target="_blank">'.stripslashes($row['box_banner']).'</a>';
				}
				else
				{
					$banner='<a href="../uploads/'.stripslashes($row['box_banner']).'" target="_blank">'.stripslashes($row['box_banner']).'</a>';
				}
			   ...
			   if($row['status'] == 1)
				...
				$output .= '<td height="30" align="center">'.$date.'</td>';
...

                                {
                                  ...
                                  $output .= '<td height="30" align="center"><button class="btn btn-info btn-sm" onclick="user_upload_copy('.$row['id'].');">Approve</button</td>';  
 ...
 // update by ample 03-02-20 & 27-10-20
	public function Add_MindJumble($page_id,$box_title,$banner_type,$banner,$box_desc,$credit_line,$credit_line_url,$sound_clip_id,$short_narration,$tags,$ref_code,$group_code,$take_key_title,$take_key_narration,$take_key_notes,$identity_type,$identity_id,$order,$is_featured)
...

	{
		...
		$ins_sql = "INSERT INTO `tblmindjumble`(page_id,`box_title`,`box_type`,`box_banner`,`box_desc`,`credit_line`,`credit_line_url`,`sound_clip_id`,`status`,`short_narration`,tags,ref_code,group_code_id,take_key_title,take_key_narration,take_key_notes,identity_type,identity_id,order_no,is_featured) VALUES ('".addslashes($page_id)."','".addslashes($box_title)."','".addslashes($banner_type)."','".addslashes($banner)."','".addslashes($box_desc)."','".addslashes($credit_line)."','".addslashes($credit_line_url)."','".addslashes($sound_clip_id)."','1','".addslashes($short_narration)."','".addslashes($tags)."','".addslashes($ref_code)."','".addslashes($group_code)."','".addslashes($take_key_title)."','".addslashes($take_key_narration)."','".addslashes($take_key_notes)."','".addslashes($identity_type)."','".addslashes($identity_id)."','".addslashes($order)."','".addslashes($is_featured)."')";
...

	{
...

		{
			...
			$group_code=stripslashes($row['group_code_id']);
			$tags=stripslashes($row['tags']);
			$take_key_title=stripslashes($row['take_key_title']);
			$take_key_narration=stripslashes($row['take_key_narration']);
			$take_key_notes=stripslashes($row['take_key_notes']);


		}

		//update by ample 19-11-19 & 03-02-20
		return array($page_id,$box_title,$banner_type,$box_desc,$banner,$status,$credit_line,$credit_line_url,$sound_clip_id,$short_narration,$order_no,$ref_code,$group_code,$tags,$take_key_title,$take_key_narration,$take_key_notes,$is_featured);
	...
	// update by ample 03-02-20
	public function Update_Mindjumble($page_id,$status,$box_title,$banner_type,$banner,$box_desc,$credit_line,$credit_line_url,$sound_clip_id,$short_narration,$banner_id,$tags,$ref_code,$group_code,$take_key_title,$take_key_narration,$take_key_notes,$order,$is_featured)
	...
	$sql = "UPDATE `tblmindjumble` SET `page_id` = '".addslashes($page_id)."', `box_title` = '".addslashes($box_title)."' , `box_type` = '".addslashes($banner_type)."', `box_banner` = '".addslashes($banner)."', `box_desc` = '".addslashes($box_desc)."',`credit_line` = '".addslashes($credit_line)."',`credit_line_url` = '".addslashes($credit_line_url)."',`status` = '".addslashes($status)."',`sound_clip_id` = '".addslashes($sound_clip_id)."' ,`short_narration` = '".addslashes($short_narration)."',
		`order_no` = '".addslashes($order)."',`tags` = '".addslashes($tags)."',`ref_code` = '".addslashes($ref_code)."' ,`group_code_id` = '".addslashes($group_code)."', `take_key_title` = '".addslashes($take_key_title)."',`take_key_narration` = '".addslashes($take_key_narration)."',`take_key_notes` = '".addslashes($take_key_notes)."',`is_featured` = '".addslashes($is_featured)."'
...

{
   ...
   // added by ample 04-12-19
    public function copy_data_tbl_icons($data="")
    {
		...
		$ins_sql = "INSERT INTO `tbl_icons`(`icons_type_id`,`identity_type`,`identity_id`,`icons_name`,`credit`,`credit_url`,`image`,`status`,`icon_tags`) VALUES ('357','user','".$data['user_id']."','".$data['box_title']."','".$data['image_video_audio_pdf_credit_line']."','".$data['image_video_audio_pdf_credit_url']."','".$data['image_video_audio_pdf']."','1','".$data['user_tags']."')";
    ...
    // added by ample 04-12-19
    public function copy_data_tblmindjumble($data="")
    {
		...
		$ins_sql = "INSERT INTO `tblmindjumble`(`box_title`,`box_type`,`box_banner`,`identity_type`,`identity_id`,`credit_line`,`credit_line_url`,`status`,`tags`) VALUES ('".$data['box_title']."','".$data['banner_type']."','".$data['image_video_audio_pdf']."','user','".$data['user_id']."','".$data['image_video_audio_pdf_credit_line']."','".$data['image_video_audio_pdf_credit_url']."','1','".$data['user_tags']."')";
	...
	// added by ample 04-12-19
    public function approved_user_uploads($id="",$admin_id="")
    {
    	$my_DBH = new mysqlConnection();
		...
		$sql = "UPDATE `tbl_user_uploads` SET `approved_by`='".$admin_id."',`approved_date`='".$date."' WHERE `id` = '".$id."'";
    ...
    // added by ample 04-12-19 & update 09-12-19
    ...
    // added by ample 05-12-19 & update 09-12-19
    public function get_data_from_tblmindjumble_OLD($box_type="",$box_id="")
    {
        $DBH = new mysqlConnection();
        $data = array();
        	if($box_type)
    ...
    //chnage date 23-12-20 
    public function get_data_from_tblsolutionitems($banner_type="",$banner_id="")
    {
        $DBH = new mysqlConnection();
        $data = array();
        	if($banner_type)
        	{
        		$sql = "SELECT * FROM  tblsolutionitems WHERE sol_item_status='1' AND banner_type='".$banner_type."'";
        	}
        	elseif($banner_id)
        	{
        		$sql = "SELECT * FROM  tblsolutionitems WHERE sol_item_id='".$banner_id."'";
        	}
        	else{
        		$sql = "SELECT * FROM  tblsolutionitems WHERE sol_item_status='1'";
        	}
            $STH = $DBH->query($sql);
            if ($STH->rowCount() > 0) {
                while ($row = $STH->fetch(PDO::FETCH_ASSOC)) {
    ...
    // add by ample 11-12-19
 //    public function getSoundClipOptions_new($sound_id="")
	// {
	// 	$my_DBH = new mysqlConnection();
 //        $DBH = $my_DBH->raw_handle();
 //        $DBH->beginTransaction();
	// 	$option_str = '';		
	// 	$sql = "SELECT * FROM `tblmindjumble` where `status` = 1 AND box_type='Sound' ";
	// 	$STH = $DBH->prepare($sql); 
 //        $STH->execute();
	// 	if($STH->rowCount() > 0)
	// 	{
	...
	// 			if($row['mind_jumble_box_id'] == $sound_id)
	// 			{
	// 				$sel = ' selected ';
	// 			}
	// 			else
	...
	// 				$sel = '';
	// 			}		
	// 			$option_str .= '<option value="'.$row['mind_jumble_box_id'].'" '.$sel.'>'.$row['box_banner'].'</option>';
	// 		}
	// 	}
	// 	return $option_str;
	// }
	//change by ample 31-12-20
	public function getSoundClipOptions_new($sound_id="")
	{
		$my_DBH = new mysqlConnection();
        $DBH = $my_DBH->raw_handle();
        $DBH->beginTransaction();
		$option_str = '';		
		$sql = "SELECT * FROM `tblsolutionitems` where `sol_item_status` = 1 AND banner_type='Sound' ";
		$STH = $DBH->prepare($sql); 
        $STH->execute();
		if($STH->rowCount() > 0)
		{
			while($row = $row = $STH->fetch(PDO::FETCH_ASSOC)) 
			{
				...
				$option_str .= '<option value="'.$row['sol_item_id'].'" '.$sel.'>'.$row['banner'].'</option>';
			}
		}
		return $option_str;
	}
	// add by ample 15-05-20
    public function get_rss_feed_title($rss_id="")
	{
		$my_DBH = new mysqlConnection();
        $DBH = $my_DBH->raw_handle();
        $DBH->beginTransaction();
		$str = '';		
		$sql = "SELECT * FROM `tblrssfeeditems` WHERE rss_feed_item_id=".$rss_id;
		$STH = $DBH->prepare($sql); 
        $STH->execute();
		if($STH->rowCount() > 0)
		{
