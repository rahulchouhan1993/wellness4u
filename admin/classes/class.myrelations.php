...
class My_Relations extends Admin
{
	public function getAllQuestions($practitioner_id,$search,$status,$country_id,$arr_state_id,$arr_city_id,$arr_place_id,$start_date)
	{
		...
		$sql = "SELECT * FROM `tblmyrelations` WHERE 1 ".$str_sql_practitioner_id."  ".$str_sql_search."  ".$str_sql_status."  ".$str_sql_country_id."  ".$str_sql_state_id."  ".$str_sql_city_id."  ".$str_sql_place_id."  ".$str_sql_date."  ORDER BY `listing_order` ASC , `mr_add_date` DESC";				
		$STH = $DBH->prepare($sql);
                $STH->execute();
		...
		$page->set_qry_string($str="mode=my_relations");
		...
		if($STH2->rowCount() > 0)
		{
			...
			while($row = $STH2->fetch(PDO::FETCH_ASSOC))
			{
				...
				$output .= '<td align="center" nowrap="nowrap">';
						if($edit) {
				$output .= '<a href="index.php?mode=edit_my_relation&id='.$row['mr_id'].'" ><img src = "images/edit.gif" border="0"></a>';
							}
				$output .= '</td>';
				$output .= '<td align="center" nowrap="nowrap">';
						if($delete) {
				$output .= '<a href=\'javascript:fn_confirmdelete("Question","sql/delmyrelation.php?id='.$row['mr_id'].'")\' ><img src = "images/del.gif" border="0" ></a>';
							}
				$output .= '</td>';
				$output .= '</tr>';
				$i++;
	...
	public function getQuestionDetails($mr_id)
	{
		$my_DBH = new mysqlConnection();
                $DBH = $my_DBH->raw_handle();
                $DBH->beginTransaction();
	...
	public function updateQuestion($situation,$situation_font_family,$situation_font_size,$situation_font_color,$listing_date_type,$days_of_month,$single_date,$start_date,$end_date,$country_id,$state_id,$city_id,$place_id,$user_id,$practitioner_id,$keywords,$listing_order,$arr_min_rating,$arr_max_rating,$arr_interpretaion,$arr_treatment,$status,$mr_id)
	{
		...
		$upd_sql = "UPDATE `tblmyrelations` SET `situation` = '".addslashes($situation)."' , `situation_font_family` = '".addslashes($situation_font_family)."' , `situation_font_size` = '".addslashes($situation_font_size)."' , `situation_font_color` = '".addslashes($situation_font_color)."' , `listing_date_type` = '".addslashes($listing_date_type)."' , `days_of_month` = '".addslashes($days_of_month)."' , `single_date` = '".addslashes($single_date)."' , `start_date` = '".addslashes($start_date)."' , `end_date` = '".addslashes($end_date)."' , `country_id` = '".addslashes($country_id)."'  , `state_id` = '".addslashes($state_id)."' , `city_id` = '".addslashes($city_id)."'  , `place_id` = '".addslashes($place_id)."' , `user_id` = '".addslashes($user_id)."' , `practitioner_id` = '".addslashes($practitioner_id)."' , `keywords` = '".addslashes($keywords)."' , `listing_order` = '".addslashes($listing_order)."' , `status` = '".addslashes($status)."'  WHERE `mr_id` = '".$mr_id."'";
	...
	public function deleteMRQuestion($mr_id)
	{
		$my_DBH = new mysqlConnection();
                $DBH = $my_DBH->raw_handle();
                $DBH->beginTransaction();
                $return=false;
		$del_sql1 = "DELETE FROM `tblmyrelations` WHERE `mr_id` = '".$mr_id."'"; 
