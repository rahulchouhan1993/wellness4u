...
//    public function updateMainSymptom($all_keyword_data_explode,$sol_item_id,$cat_total_cnt1,$keywords,$symptom_keyword_id,$sym_code,$id,$bms_name,$keywordsold,$total_count,$comment,$hdnbmsid,$fav_cat_type_id,$fav_cat_id_parent,$show_hide,$symtum_status)
    public function updateMainSymptom($id,$sol_item_id,$bms_name,$keywords,$sym_code,$symptom_id,$fav_cat_type_id,$fav_cat_id_parent,$comment,$show_hide,$symtum_status,$key_fav_cat_type_id,$key_fav_cat_id,$sub_cat_link)     
    {
...
//                    $option_str .= '<option value="'.$row['kw_id'].'" '.$sel.'>'.stripslashes($kw_name).'</option>';
    ...
    public function addMainSymptomRamakant($sub_cat_link,$key_fav_cat_id,$key_fav_cat_type_id,$keywords,$cat_total_cnt1,$daily_code,$bms_name,$comment,$show_hide,$arr_cucat_parent_cat_id,$arr_cucat_cat_id,$cat_total_cnt)
    {
      ...
      public function addKeywordcustomSymptom($tdata_cat)
            ...
            $DBH->beginTransaction();
            $return = false;
            $sql = "INSERT INTO `tbl_symptom_keyword` (`symptom_id`,`keywords`,`key_prof_cat`,`key_sub_cat`,`fetch_link`) VALUES ('".addslashes($tdata_cat['symptom_id'])."','".addslashes($tdata_cat['keyword'])."','".($tdata_cat['key_prof_cat'])."','".($tdata_cat['key_sub_cat'])."','".addslashes($tdata_cat['fetch_link'])."')";
            $STH = $DBH->prepare($sql);
            $STH->execute();
            if($STH->rowCount() > 0)
            {
