...
class Daily_Activity extends Admin
{
	...
	public function getActivityDetails($activity_id)
	{
		...
		return array($activity_code,$activity,$activity_cal_kg_min,$activity_cal_kg_hr,$activity_cal_59_kg,$activity_level_code,$activity_category,$recommendations,$guidelines,$precautions,$benefits);
	}
	
	
	public function addDailyActivity($activity_code,$activity,$activity_cal_kg_min,$activity_cal_kg_hr,$activity_cal_59_kg,$activity_level_code,$activity_category,$recommendations,$guidelines,$precautions,$benefits)
	{
		$my_DBH = new mysqlConnection();
                $DBH = $my_DBH->raw_handle();
                $DBH->beginTransaction();
		...
		$sql = "INSERT INTO `tbldailyactivity` (`activity_code`,`activity`,`activity_cal_kg_min`,`activity_cal_kg_hr`,`activity_cal_59_kg`,`activity_level_code`,`activity_category`,`recommendations`,`guidelines`,`precautions`,`benefits`) VALUES ('".addslashes($activity_code)."','".addslashes($activity)."','".addslashes($activity_cal_kg_min)."','".addslashes($activity_cal_kg_hr)."','".addslashes($activity_cal_59_kg)."','".addslashes($activity_level_code)."','".addslashes($activity_category)."','".addslashes($recommendations)."','".addslashes($guidelines)."','".addslashes($precautions)."','".addslashes($benefits)."')";
		$STH = $DBH->prepare($sql);
                $STH->execute();
		if($STH->rowCount() > 0)
		{
	...
	public function updateDailyActivityFull($activity_code,$activity,$activity_cal_kg_min,$activity_cal_kg_hr,$activity_cal_59_kg,$activity_level_code,$activity_category,$recommendations,$guidelines,$precautions,$benefits,$activity_id)
	{
		$my_DBH = new mysqlConnection();
                $DBH = $my_DBH->raw_handle();
                $DBH->beginTransaction();
