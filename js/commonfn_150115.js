...
function getModuleWiseCriteriaValueInputField()
{
    $('#divreportresults').hide();
    var report_module = $('#report_module').val();
    var pro_user_id = $('#pro_user_id').val();
    ...
    startPageLoading();
    link='remote.php?action=getmodulewisecriteriavalueinputfield&module_criteria='+module_criteria+'&report_module='+report_module+'&pro_user_id='+pro_user_id;
        var linkComp = link.split( "?");
        var result;
        var obj = new ajaxObject(linkComp[0], fin);
        obj.update(linkComp[1],"GET");
        obj.callback = function (responseTxt, responseStat) {
                // we'll do something to process the data here.
                result = responseTxt;
                $('#tdcriteriavalueresult').html(result);
                stopPageLoading();
        }
}
...
function showDeclineAdviserInvitationPopup(ar_id,puid)
{
	//startPageLoading();
	link='remote.php?action=showdeclineadviserinvitationpopup&ar_id='+ar_id+'&puid='+puid;
	var linkComp = link.split( "?");
	var result;
	var obj = new ajaxObject(linkComp[0], fin);
	obj.update(linkComp[1],"GET");
