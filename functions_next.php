...

{
	...
	$output .= '<div style="float:left;padding-left:10px;margin-top:15px;width:800px;height:500px;overflow:scroll;">';

	$output .= '	<table width="800" border="1" cellpadding="0" cellspacing="0" class="report" style="border-collapse:collapse;">
...
function getparsonalitiesDetails()

{

	global $link;
	$image = '';
	$sql = "SELECT * FROM `tbl_personalities` WHERE 1";
	//echo '<br>Testkk: sql = '.$sql;
	$result = mysql_query($sql,$link);
	if( ($result) && (mysql_num_rows($result) > 0) )
	{
...
function getMyActivityCountByDate($date, $user_id) {
...
function getMyFoodCountByDate($date, $user_id) {
...
function getMySituationByDate($date, $user_id) {
...
function getMyCommunicationsByDate($date, $user_id) {
...
function getMyWorkplaceByDate($date, $user_id) {
...
function getMyAddictionsByDate($date, $user_id) {
 ...
 $sql = "SELECT * FROM tblusersadct where user_id = ".$user_id." and adct_date = '".$date."'"; 
...
function getAllIconsDetailsVivek()

{
	...
	$sql = "SELECT * FROM `tbl_icons` WHERE `display_type`='381'";
...
function getAllIconsDisplayTypeDetailsVivek()

{
	...
	$sql = "SELECT * FROM `tbl_icons` WHERE `display_type`='382' and `status`='1' ORDER BY `order` ASC";
...
function getSubCatNameById($favcat_id)
...
function getThemeDataValueVivek($day)

{
	...
	$sql = "SELECT * FROM `tbltheams` WHERE status = '1'";
...
function getThemeImageByImageIdVivek($theme_id)

{
	...
	$sql = "SELECT * FROM `tbltheams` WHERE theam_id = '".$theme_id."'";
...
function getAllIconsDetailsValueVivek($day)

{
	...
	$sql = "SELECT * FROM `tbl_icons` WHERE 1";
...
function getIconImageByImageIdVivek($icon_id)

{
	...
	$sql = "SELECT * FROM `tbl_icons` WHERE icons_id = '".$icon_id."'";
...
function getKeyworIdByKeywordNameFromTblBodymainsymtom($bms_name)

{
	...
	$sql = "SELECT * FROM `tblbodymainsymptoms` WHERE `bms_name` = '".$bms_name."'";
...

	{
		...
		$page_name = stripslashes($row['bms_id']);
...
function getSelectedSolIdByChkKeywordVivek($keyword_name)

{
	...
	$data = '';
        ...
        for($i=0; $i<count($keyword_name);$i++)
        {
	
           $sql = "SELECT * FROM `tbl_wellness_solution_item_keyword` WHERE `keyword_name` LIKE'%".$keyword_name[$i]."%'";
...
function getAllIconsDetailsBySolItemIdVivek($sub_fav_cat_id)

{
	...
	$data = '';
	 ...
	 $sql = "SELECT * FROM `tbl_icons` WHERE `display_type`='381' and `fav_cat_id` IN('".$sub_fav_cat_id."') and `status`='1' Group By `fav_cat_id` Order By `icons_id` DESC";
...

	{
		...
		while($row = mysql_fetch_array($result))
                {
                  $day=date('d');
                  $days_of_month=explode(',',$row['days_of_month']);
                  if(in_array($day,$days_of_month))
                  {
		    $data[] = $row;
                  }
                }
...
function getAllIconsIdDetailsBySolItemIdVivek($sub_fav_cat_id)

{
	...
	$sql = "SELECT DISTINCT(`fav_cat_id`) FROM `tbl_icons` WHERE `display_type`='381' and `fav_cat_id` IN('".$sub_fav_cat_id."') ORDER BY `fav_cat_id` DESC";
...
function chkSelectedSolItemIdByFavCatIdVivek($sol_item_id)

{
	...
	$sql = "SELECT * FROM `tblsolutionitems` WHERE `sol_item_id`IN('".$sol_item_id."') and `sol_item_deleted` = 0 and `sol_item_status` = 1";
...
function getSelectedSolIdByChkSolidAndFavCatVivek($sol_item_id_implode_value,$fav_cat_id)
...
function getWSIDataByWSIReferenceNumberVivek($ref_num_implode,$sol_item_id)

{
	...
	$data = '';
         ...
         $sql = "SELECT * FROM `tblsolutionitems` WHERE `wellbgn_ref_num`IN('".$ref_num_implode."') and `sol_item_status` = '1' and `sol_item_deleted` = '0' ";
...

	{
        ...
        $sql = "SELECT * FROM `tblsolutionitems` WHERE `sol_item_id`IN('".$sol_item_id."')  and `sol_item_status` = '1' and `sol_item_deleted` = '0'";
...
function getWSIAllDataByWSIReferenceNumberVivek($ref_num_implode,$sol_item_id)
...
function getWSIAllDataByWSISolIDAndFavCatVivek($sol_id_implode,$fav_cat_id)
		...
		while($row = mysql_fetch_array($result))
                {
                    if($_SESSION['ref_num']!='')
                    {
                        if($_SESSION['ref_num'] == $row['wellbgn_ref_num'])
                        {
                          $data[] = $row;  
                        }
                        else
...
function getGuideAndTipsDataVivek()

{
	...
	$data = '';
	 ...
	 $sql = "SELECT * FROM `tbl_icons` WHERE fav_cat_id = '24' ORDER BY `icons_id` DESC";
...
function getSelectedSolItemKeywordIdByChkKeywordVivek($keyword_name)
...
function getTblSolutionDataBySolIdVivek($sol_item_id)

{
	...
	$data = '';
	 ...
	 $sql = "SELECT * FROM `tblsolutionitems` WHERE `sol_item_id` = '".$sol_item_id."' and `sol_item_status` = '1' and `sol_item_deleted` = '0'";
...
function getLikeData($user_id,$sol_item_id)

{
	...
	$data = '';
        ...
        $sql = "SELECT * FROM `tbl_chatt_box` WHERE `user_id` = '".$user_id."' and `sol_item_id` = '".$sol_item_id."'";
...
function getWSIAllDataByWSIReferenceNumberVivekRemote($keyword,$sol_item_id)

{
	...
	$sql = "SELECT * FROM `tblsolutionitems` WHERE `wellbgn_ref_num`IN('".$keyword."') and `sol_item_status` = '1' and `sol_item_deleted` = '0'";
...
function addUsersSleepQuestionByGestVivek($user_id,$sleep_date,$selected_sleep_id_arr,$scale_arr,$remarks_arr,$my_target_arr,$adviser_target_arr,$sleep_set_id)
