...
elseif ($action == 'feedback_form_update') {
    ...
    $reward_type = stripslashes($_REQUEST['reward_type']);
    $error = '0';
    $err_msg = '';
    $output = '';
    $output = $obj2->showRewardCatlog($module_id,$reward_type);
    echo $output;
...
{   
    ...
    $edate= date("Y-m-d", strtotime($_POST['end'])) ;
        ...
        $date_range=$obj->get_date_between_range($sdate,$edate);
        ...
        $event1 = array();
            foreach ($data1 as $key => $value) {
                ...
                $token=base64_encode($title);
                $url='design-my-life-data-view.php?data_id='.$value['id'].'&token='.$token;

                $time=trim($value['bes_time']);

                $color="";
                    if($value['is_repeat_task']==1)
                    {
                        $color='red';
                    }
                ...
                {    
                    ...
                    $action=$obj->task_do_it($value['id'],$value['single_date'],'parent');
                    if($action==true)
                    {
                        $color='gray';
                    } 
                    ...
                    if($time)
                    {
                             $event1[]=array('title' =>$title,
                                'url' =>$url,
                                'start'=>$value['single_date'].'T'.$time,
                                'color'=>$color,
                            );
                    }
                    else
                ...
                {   
                    ...
                    foreach ($date_range as $index => $row) {
                        ...
                        if($row>=$value['start_date'] && $row<=$value['end_date'])
                        {   
                            $action=$obj->task_do_it($value['id'],$row,'parent');
                            if($action==true)
                            {
                                $color='gray';
                            } 
                ...
                {   
                    $weeks=explode(',', $value['days_of_week']);
                    $date = date('Y-m-d', strtotime($value['add_date']));
            ...
            foreach ($data3 as $key => $value) {
                ...
                {   
                    foreach ($date_range as $index => $row) {
                        ...
                        if($row>=$value['start_date'] && $row<=$value['end_date'])
                        {   
                            $action=$obj->task_do_it($value['input_id'],$row,'child');
                            if($action==true)
                            {
                                $color='gray';
                            } 
...
{   
    ...
    $html='<form action="" id="scrollBarForm">
            <div class="form-group"><label>Priority:</label><select class="form-control" id="priority" required>'.$obj->getManageFavCatDropdownDataOption(37).'</select>
            </div>
            <div class="form-group"> <label for="comment">Note:</label><textarea class="form-control" rows="5" id="fav_note" name="note" required></textarea></div>
    ...
    {   
        ...
        foreach ($data as $key => $value) {
            if($value['banner_type']=='Image')
            {
                ...
                {
                    $banner_data='<div class="zoomonhoverul"><div class="zoomonhover"><a href="'.SITE_URL.'/uploads/'.$imgData['image'].'" class="preview"><img src="'.SITE_URL.'/uploads/'.$imgData['image'].'" style="width:300px; height: 175px;object-fit: contain;" class="img-responsive" alt="" /></a></div></div>';
                }  
                else
                {
                     $banner_data='<div class="zoomonhoverul"><div class="zoomonhover"><a href="'.SITE_URL.'/images/no-image.png" class="preview"><img src="'.SITE_URL.'/images/no-image.png" style="width:300px; height: 175px;object-fit: contain;" class="img-responsive" alt="" /></a></div></div>';
                }
            ...
            {   
                ...
                {
                    $banner_data='<iframe width="275px" height="175px" src="'.$fileData["banner"].'" frameborder="0" allowfullscreen></iframe>';
                }
                else if($value['banner_type']=='Sound' || $value['banner_type']=='Audio')
                {
                    $banner_data='<embed src="'.SITE_URL.'/uploads/'.$fileData["banner"].'" autostart="true" loop="true" height="175px" width="275px"></embed>';
                }
                else if($value['banner_type']=='Pdf')
                {
                    $banner_data='<a href="'.SITE_URL.'/uploads/'.$fileData['banner'].'" target="_blank"><img src="'.SITE_URL.'/images/pdf-icon.png" style="width:300px; height: 175px;object-fit: contain;" class="img-responsive" alt="" /></a>';
                }
                else
                {
                    $banner_data='<div class="zoomonhoverul"><div class="zoomonhover"><a href="'.SITE_URL.'/images/no-image.png" class="preview"><img src="'.SITE_URL.'/images/no-image.png" style="width:300px; height: 175px;object-fit: contain;" class="img-responsive" alt="" /></a></div></div>';
                }
                $url_file=$fileData['banner'];
            }
             ...
             if(!empty($button_data))
                {
                  ...
                  foreach ($button_data as $b_key => $b_value) {
                        if($b_value['popup_type']==1)
                        {
                            $btn='<button class="btn" onclick="openFavLibraryBox(\''.$value["banner_type"].'\',\''.$url_file.'\',\''.$main_data['topic_subject'].'\',\''.$cat_id.'\',\'0\',\' \',\'Favourite\')" style="color:#'.$b_value['font_color'].';background-color: #'.$b_value['bg_color'].'" title="'.$b_value['button_heading'].'"><i class="fa '.$b_value['icon_code'].'"></i> </button>';
                        }
                    }
                }
            ...
            $html.='<div class="library-box text-center">'.$banner_data;
                    ...
                    {
                        $html.='<p><a href="'.$value['credit_url'].'" target="_blank">'.$value['credit'].'</p></a>'; 
                    }
        ...
        $html.='</div>';
        $html.='<script type="text/javascript"> imagePreview(); </script>';
    }
    else
    {
...
//add by ample 10-6-20
elseif($action == 'openFavLibraryBox')
{   
    $type = $_REQUEST['type'];
    $file = $_REQUEST['file'];
    ...
    $html='<form action="" id="FavLibraryForm">
            <div class="form-group"><label>Priority:</label><select class="form-control" id="priority" required>'.$obj->getManageFavCatDropdownDataOption(37).'</select>
            </div>
            <div class="form-group"> <label for="comment">Note:</label><textarea class="form-control" rows="5" id="fav_note" name="note"></textarea></div>
            <input type="hidden" name="type" id="type" value="'.$type.'"/><input type="hidden" name="file" id="file" value="'.$file.'"/><input type="hidden" name="title" id="title" value="'.$title.'"/><input type="hidden" name="category" id="category" value="'.$category.'"/><input type="hidden" name="type_id" id="type_id" value="'.$type_id.'"/><input type="hidden" name="section" id="section" value="'.$section.'"/><input type="hidden" name="data_type" id="data_type" value="'.$data_type.'"/>
            <button type="button" class="btn btn-default" onclick="saveFavLibraryBoxData()">Submit</button>
            </form>';
...
elseif($action == 'saveFavLibraryBoxData')
{ 
    ...
    $res=$obj->saveFavLibraryBoxData($title,$category,$note,$type,$file,$type_id,$section,$priority,$data_type);

    echo $res;

}
//add by ample 06-10-20
elseif($action == 'Delete_FavLibrary')
{ 
    
    $id = $_REQUEST['id'];

    $res=$obj->delete_FavLibrary_data($id);
...
//add by ample 14-08-20
elseif($action == 'Update_FavLibraryData')
{   
    $id = $_REQUEST['id'];
    $lib_data=$obj->get_short_favlibrary_data($id);
    $html='<form action="" id="scrollBarForm">
            ...
            <button type="button" class="btn btn-default" onclick="UpdateFavLibraryData('.$id.')">Submit</button>
            </form>';
...
//add by ample 14-08-20
elseif($action == 'UpdateFavLibraryData')
{ 
    
    $id = $_REQUEST['id'];
    $note = $_REQUEST['fav_note'];

    $res=$obj->update_favlibrary_list_note($id,$note);
...
{   
    ...
    //$box_data=$obj->get_reward_list_box_data($page_id,$button_id); //update date 16-12-20
    $box_data=$obj->get_reward_list_box_data_on_schedule($page_id,$button_id);
    ...
    {   
        ...
        foreach ($box_data as $key => $value) {
            ...
            $banner_data=$obj->get_common_gallery_file($value['reward_list_file_type'],$value['reward_list_file']);

            $html.='<div class="reward-box"><div style="width: 150px; height: 150px; margin: auto;overflow: auto;">'.$banner_data.'</div> <p>'.$value['reward_list_name'].'</p> <p>'.$value['reward_title_remark'].'</p> <br></div>';
...
elseif($action=='today_event_task')
{  
   $_SESSION["today_task_popup"] = 1; 
   ...
   {
        $today=date('Y-m-d');
        $html.='<table class="table"><thead><tr><th>Sr.no.</th><th>Today Task</th><th>Status</th><th>Action</th></tr></thead><tbody>';
        foreach ($events as $key => $value) {
                ...
                {
                   $action=$obj->task_do_it($value['id'],$today,$value['type']); 
                }
                elseif ($value['type']=='child') {
                   $action=$obj->task_do_it($value['sub_id'],$today,$value['type']);
                }
               
              $html.='<tr><td>'.$index.'</td><td>'.$title.'</td>';  
              if($action==true)
              {
                $html.='<td><span class="text-success">Done</span></td>';
              }
              else
              {
                $html.='<td><span class="text-danger">Due</span></td>';
              }
              $html.='<td><a href="'.$url.'" target="_blank"><button class="btn btn-default"><i class="fa fa-eye"></i></button></a></td> </tr>';
        }
            $html.='</tbody></table>';
   }
   else
   {
     $html="No today task found!";
   }
   echo $html;
}
elseif ($action=='check_do_it_history') {
    $type=$_POST['type'];
    $id=$_POST['id'];
    $title=$_POST['title'];
    $html="<h3>".$_POST['title']."<button class='btn btn-default btn-sm pull-right' style='margin-top: -5px;' onclick='repeat_task(\"".$_POST['title']."\")'>Repeat</button></h3><hr>";
    $history=$obj->task_do_it_history($id,$type);
    if(!empty($history))
    {   
        $html.='<table class="table"><thead><tr><th>Sr.no.</th><th>Date</th></tr></thead><tbody>';
        foreach ($history as $key => $value) {
            $index=$key+1;
            $html.='<tr><td>'.$index.'</td><td>'.date("d M Y", strtotime($value)).'</td></tr>';
        }
        $html.='</tbody></table>';
    }
    else
       {
         $html.="<br>No history found!";
       }
    echo $html;
}
elseif ($action=='repeat_task_form') {

    $title=$_POST['title'];
    $html='<form action="" method="post"><div class="form-group"><label>Task Title</label> <input type="text" class="form-control" value="'.$title.'" name="title" readonly></div>
          <div class="form-group"><label class="radio-inline"><input type="radio" name="dateradio" value="single_date" class="date_type_change" checked>Single Date</label><label class="radio-inline"><input type="radio" name="dateradio" class="date_type_change" value="date_range">Date Range</label></div>
          <div class="form-group row" id="single_date_box"><div class="col-md-6"><label> Date</label><input type="text" class="form-control" name="date" placeholder="Enter Date" id="single_date_new" autocomplete="off"></div></div>
          <div class="form-group row" id="date_range_box" style="display:none;"><div class="col-md-6"><label>Start Date</label><input type="text" class="form-control" name="start_date" placeholder="Enter Start Date" id="start_date_new" autocomplete="off"></div><div class="col-md-6"><label>End Date</label><input type="text" class="form-control" name="end_date" placeholder="Enter End Date" id="end_date_new" autocomplete="off"></div></div>
          <button type="submit" name="new_task" class="btn btn-default">Save</button>
        </form>';
    $html.="<script> $('.date_type_change').change(function(){if($(this).val() == 'single_date'){ $('#single_date_box').show(); $('#date_range_box').hide();} else { $('#single_date_box').hide(); $('#date_range_box').show();} });$('#single_date_new').datepicker({ minDate: 0 , dateFormat : 'dd-mm-yy'}); $('#start_date_new').datepicker({ minDate: 0 , dateFormat : 'dd-mm-yy'});$('#end_date_new').datepicker({ minDate: 0 , dateFormat : 'dd-mm-yy'});</script>";
    echo $html;
}
//add by ample 21-08-20
elseif($action == 'open_reward_redeem_box')
{   
    $user_id = $_REQUEST['user_id'];
    ...
    $html='<form action="#" id="point_redeem_form" method="post">
              <div class="form-group">
                <label>Your Points:</label>
                <input type="text" class="form-control" value="'.$my_points.'" id="my_point" readonly>
              </div>
              <div class="form-group">
                <label>Enter Points:</label>
                <input type="number" class="form-control" id="redeem_point" >
              </div>
...
elseif ($action=='get_module_points') {
   $module_id = $_REQUEST['module_id'];
   $user_id = $_SESSION['user_id'];
   $total_points=$obj->get_total_points($user_id,$module_id);
   echo $total_points;
}
//add by ample 18-11-20
elseif ($action=='get_reward_list_data') {
   $module_id = $_REQUEST['module_id'];
   $points = $_REQUEST['points'];
   $list=$obj->get_reward_list($module_id,$points);
    ...
    if(!empty($list))
    {
        foreach ($list as $key => $value) {
            $html.='<div class="radio">
                      <label><input type="radio" name="prize_id" value="'.$value["reward_list_id"].'" checked>'.$value["reward_list_name"].'</label>
                    </div>';
        }
    }
    else
    {
       $html="No reward prize available try another module"; 
    }

    echo $html;
}
//add by ample 18-11-20
elseif ($action=='point_redeem_form_submit') {

    $user_id = $_SESSION['user_id'];
    $uniqu_id=uniqid();
    $user_points=$obj->get_user_points($user_id);
    $user_points=$user_points-$_REQUEST['redeem_point'];
    $new_data = array( 'user_id' => $user_id,
                        'reward_module_id'=>$_REQUEST['module_id'],
                        'points'=>$_REQUEST['redeem_point'],
                        'transaction_type'=>'Debit',
                        'reward_scheme_type'=>'reward_prize',
                        'reward_scheme_id'=>$_REQUEST['prize_id'],
                        'remark'=>'Reward Redeem',
                        'transection_no'=>$uniqu_id,
                        'balance'=>$user_points
                     );

    $res=$obj->added_reward_history($new_data);

    if($res==true)
    {
        $obj->update_user_points($user_id,$user_points);
    }
    echo $res;
}
//add by ample 19-11-20
elseif ($action=='get_reward_list_data_all_module') {
   $points = $_REQUEST['points'];
   $list=$obj->get_reward_list_all_module($points);
    ...
    if(!empty($list))
    {
        foreach ($list as $key => $value) {
            $html.='<div class="radio">
                      <label><input type="radio" name="prize_ids" value="'.$value["reward_list_id"].'" checked>'.$value["reward_list_name"].'</label>
                    </div>';
        }
    }
    else
    {
       $html="No reward prize available try later"; 
    }

    echo $html;
}
//add by ample 19-11-20
elseif ($action=='point_redeem_form_submit_all_module') {

    $result=false;
   
    $user_id = $_SESSION['user_id'];
    $selected_module_id=$_REQUEST['selected_reward_module_id'];
    $my_point=$_REQUEST['my_point'];
    ...
    {
        foreach ($selected_module_id as $key => $value) {

            $user_points=$obj->get_user_points($user_id);
            $user_points=$user_points-$redeem_point[$value];

            $new_data = array( 'user_id' => $user_id,
                        'reward_module_id'=>$module_id[$value],
                        'points'=>$redeem_point[$value],
                        'transaction_type'=>'Debit',
                        'reward_scheme_type'=>'reward_prize',
                        'reward_scheme_id'=>$_REQUEST['prize_ids'],
                        'remark'=>'Reward Redeem',
                        'transection_no'=>$uniqu_id,
                        'balance'=>$user_points
                     );
...
//add by ample 19-11-20
elseif ($action=='buy_user_subcription_plan') {
    ...
    $prize_id=$_REQUEST['prize_id'];

    $plan_data=$obj->get_user_plan_data($plan_id);

     $new_data = array( 'user_id' => $user_id,
                    'up_id'=>$plan_id,
                    'uup_amount'=>$plan_data['up_amount'],
                    'uup_currency'=>$plan_data['up_currency'],
                    'uup_duration'=>$plan_data['up_duration'],
                    'uup_points'=>$plan_data['up_points'],
                    'plan_prize_id'=>$prize_id,
                    'uup_payment_status'=>'Pending',
                    'uup_payment_mode'=>$pay_mode,
                    'uup_payment_details'=>$pay_info,
                    'upr_status'=>0
                 );

        $res=$obj->send_plan_request_admin($new_data);

    echo $res;

}
//add by ample 27-11-20
elseif($action == 'confirm_user_subcription_plan_form')
{   
    $user_id = $_REQUEST['user_id'];
    $plan_id=$_REQUEST['plan_id'];
    $plan_data=$obj->get_user_plan_data($plan_id);

    $prize_ids=explode(',', $plan_data['prize_list']);

    $html='<div class="text-center"><b>'.$plan_data['up_name'].'</b> <br> <b>'.$plan_data['up_amount'].' '.$obj->getFavCode($plan_data['up_currency']).'</b><br><b>'.$plan_data['up_duration'].' Days</b></div>';


    $html.='<form action="#" id="plan_request_form" method="post">
              <div class="form-group">
                <label>Payment Mode:</label>
                <select name="payment_mode" id="up_payment_mode" class="form-control">
                    <option value="">Select Payment Mode</option>
                    '.$obj->getFavCategoryRamakant(96,'').'
                </select>
              </div>
              <div class="form-group"><label>Payment Details:</label><textarea name="payment_details" id="up_payment_details" class="form-control"></textarea></div>';

                if(!empty($prize_ids))
                {
                    $html.='<div class="form-group">
                    <label>'.$plan_data['prize_heading'].'</label></div>';
                    foreach ($prize_ids as $prize_id) {
                        $prize_data=$obj->get_reward_prize_data($prize_id);
                        $html.='<div class="radio"><label><input type="radio" name="prize_id" id="plan_prize_id" value="'.$prize_id.'">'.$prize_data['reward_list_name'].'</label></div>';
                    }
                }
        $html.='<input type="hidden" name="up_id" id="up_id" value="'.$plan_id.'" />
              <button type="button" onclick="buy_plan('.$plan_id.')" class="btn btn-default">Request Now</button>
            </form>';

    echo $html;
}
elseif ($action == 'get_referal_code') 
{   
    $email_id = $_REQUEST['email_id'];
    echo $obj->get_referal_id($email_id);
}
elseif ($action == 'get_referal_name') 
{   
    $code = $_REQUEST['code'];
    if(empty($code))
    {
    ...
    {
        $name=$obj->get_user_name_by_referenceID($code);
        if(empty($name))
        {
           $name=$obj->get_vendor_name_by_referenceID($code); 
        }
        echo 'Reference: '.$name;
    }
    
}elseif ($action == 'getheightandwidth') {   
    $DBH = new DatabaseHandler();
    $positionId = $_REQUEST['position_id'] ?? 0;
    
    $sqlQuery = "SELECT * FROM `tblposition` WHERE `position_id`='" . $positionId . "'";
    $queryObject = $DBH->query($sqlQuery);
    $fetchedRecord = $queryObject->fetch(PDO::FETCH_ASSOC);
    $height = $fetchedRecord['height'] ?? 0;
    $width = $fetchedRecord['width'] ?? 0;
