...
$button_data=$obj->get_common_button_setting_data('Page',$page_id);
...
if(!empty($button_data))
{
  ...
  <br>
  <div class="row">
    <div class="col-md-12 reward-button-section">
      <?php 
      foreach ($button_data as $key => $value) {
                          ...
                          {
                            if($value['banner_type']=='Image' && !empty($value['banner']))
                                {   
                                    ...
                                    if(!empty($imgData['image']))
                                    {
                                        ...
                                        $img='<img src="uploads/'.$imgData['image'].'" class="reward-img" style="background-color: #'.$value['bg_color'].';border-color: #'.$value['font_color'].';">';
        ...
        ?>
          <button class="btn btn-default btn-block reward-btn" onclick="view_reward('<?=$page_id;?>','<?=$value['button_name'];?>')" style="background-color: #<?=$value['bg_color'];?>;color: #<?=$value['font_color'];?>" title="<?=$value['button_heading'];?>"><i class="fa <?=$value['icon_code'];?>"></i> <?=$value['button_heading'];?> <?=$img;?></button>
        <?php
      }
      ?>
    </div>
...
if($obj->chkIfScrollingWindowActivateForPage($page_id))

{ 

 echo $obj->getScrollingWindowsCode($page_id);
...
list($arr_banner_id,$arr_page_id,$arr_page,$arr_position_id,$arr_banner,$arr_url,$arr_banner_type,$arr_position,$arr_side,$arr_width,$arr_height) = $obj->getbanners($page_id,'Left');
