...
if(isset($_GET['sess']) && $_GET['sess'] != '')
{
	...
	{
		if(doValiadteProUser($email))

		{

			$pro_user_id = getProUserId($email);
			$name = getProUserFullNameById($pro_user_id);
			$unique_id = getProUserUniqueId($pro_user_id);

			$err_msg = 'Welcome '.$name.'! You\'re now registered member  <a href="'.SITE_URL.'/prof_login.php">Click here to Sign In</a>';		
