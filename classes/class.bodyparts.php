...
//        $sql = "SELECT * FROM `tblsymtumscustomcategory`,`tblbodymainsymptoms` WHERE tblsymtumscustomcategory.bmsid = tblbodymainsymptoms.bms_id and symtum_deleted = '0' ".$str_sql_search." ".$str_sql_status." ".$str_sql_bmst_id." ORDER BY tblbodymainsymptoms.bms_name ASC ";		
        $sql = "SELECT * FROM `tblsymtumscustomcategory` WHERE  symtum_deleted = '0' ".$str_sql_search." ".$str_sql_status." ".$str_sql_bmst_id." ORDER BY bmsid DESC ";		
        ...
        if($STH2->rowCount() > 0)
        {
            ...
            while($row = $STH2->fetch(PDO::FETCH_ASSOC))
            {
                ...
                $output .= '<td height="30" align="center">'.$obj->getKeywordsNameByFavCarVivek($keywords_data_implode).'</td>';
                $output .= '<td height="30" align="center">'.$bms_status.'</td>';
...
//        }
    ...
    public function getMainSymptomDetailsRamakant($id)
	{
           ...
           //SELECT table1.*, table2.col1, table2.col3 FROM table1 JOIN table2 USING(id)
            ...
            return array($sol_item_id,$sym_code,$bms_name,$bmsid,$fav_cat_type_id,$fav_parent_cat,$keywords,$symtum_status,$show_hide,$comment);
	}
